...
// electron/shared/office-table-source-binding.mjs
var office_table_source_binding_exports = {};
__export(office_table_source_binding_exports, {
  bindOfficeTableSources: () => bindOfficeTableSources,
  getOfficeDescriptorByteLimit: () => getOfficeDescriptorByteLimit
});
...
function normalizeSemanticText(value) {
  return value.replace(/\r\n?|\n/g, "\n").normalize("NFC").split("\n").map((line) => line.replace(COLLAPSIBLE_SPACE_PATTERN, " ").replace(EDGE_SPACE_PATTERN, "")).join("\n");
}
function htmlSemanticText(value) {
  ...
  const visit = (node2) => {
    ...
    if (node2.nodeName === "br") result += "\n";
    if (Array.isArray(node2.childNodes)) node2.childNodes.forEach(visit);
  };
  fragment.childNodes.forEach(visit);
  return result;
}
function mdastSemanticText(node2) {
  if (!node2 || typeof node2 !== "object") return "";
  if (node2.type === "text" || node2.type === "inlineCode") {
    return typeof node2.value === "string" ? node2.value : "";
  }
  if (node2.type === "break") return "\n";
  if (node2.type === "image" || node2.type === "imageReference") return "";
  if (node2.type === "html") {
    return typeof node2.value === "string" ? htmlSemanticText(node2.value) : "";
  }
  return Array.isArray(node2.children) ? node2.children.map(mdastSemanticText).join("") : "";
}
function collectTables(root) {
  const tables = [];
  const visit = (node2) => {
    if (!node2 || typeof node2 !== "object") return;
    if (node2.type === "table") tables.push(node2);
...
function bindParsedTable(table, markdown, tableIndex) {
  ...
  if (!Number.isInteger(startOffset) || !Number.isInteger(endOffset) || startOffset < 0 || endOffset > markdown.length || startOffset >= endOffset) {
...
async function bindOfficeTableSources(markdown, sha256Hex) {
  ...
  const parsedTables = collectTables(tree).map((table, tableIndex) => bindParsedTable(table, markdown, tableIndex));
...
function getOfficeDescriptorByteLimit(bodyUtf8Bytes, tableCount, totalLogicalColumns) {
  ...
  const limit = 512n + BigInt(bodyUtf8Bytes) + 512n * BigInt(tableCount) + 32n * BigInt(totalLogicalColumns);
  return limit > BigInt(Number.MAX_SAFE_INTEGER) ? Number.MAX_SAFE_INTEGER : Number(limit);
}
var SHA256_PATTERN, COLLAPSIBLE_SPACE_PATTERN, EDGE_SPACE_PATTERN;
var init_office_table_source_binding = __esm({
  "electron/shared/office-table-source-binding.mjs"() {
    ...
    COLLAPSIBLE_SPACE_PATTERN = /[\u0009\u000c\u0020\u00a0]+/g;
    EDGE_SPACE_PATTERN = /^[\u0009\u000c\u0020\u00a0]+|[\u0009\u000c\u0020\u00a0]+$/g;
  }
});

// electron/shared/office-table-presentation-validation.cjs
var require_office_table_presentation_validation = __commonJS({
  "electron/shared/office-table-presentation-validation.cjs"(exports2, module2) {
    "use strict";
    var INVALID_TABLE_PRESENTATION2 = "INVALID_TABLE_PRESENTATION";
    var TABLE_PRESENTATION_MISMATCH = "TABLE_PRESENTATION_MISMATCH";
    var HASH_PATTERN = /^[0-9a-f]{64}$/;
    var COLOR_PATTERN = /^#[0-9a-f]{6}$/;
    var DESCRIPTOR_KEYS = ["markdownSha256", "tables"];
    var TABLE_KEYS = [
      "tableIndex",
      "sourceSha256",
      "logicalWidth",
    ...
    function presentationError(code2) {
      const error = new Error(code2);
      error.code = code2;
      return error;
    }
    function failInvalid() {
      throw presentationError(INVALID_TABLE_PRESENTATION2);
    }
    ...
    function exactEnumerableKeys(value, expected) {
      ...
      if (allOwnKeys.some((key) => typeof key === "symbol") || allOwnKeys.length !== keys.length || keys.length !== expected.length || expected.some((key) => !Object.hasOwn(value, key)) || keys.some((key) => !expected.includes(key))) failInvalid();
    }
    function denseArray(value) {
      if (!Array.isArray(value)) failInvalid();
      for (let index2 = 0; index2 < value.length; index2 += 1) {
        if (!Object.hasOwn(value, index2)) failInvalid();
      }
      const keys = Reflect.ownKeys(value);
      if (keys.some((key) => typeof key === "symbol" || key !== "length" && !/^(0|[1-9][0-9]*)$/.test(key))) failInvalid();
    }
    ...
    function hasOfficePresentationFields2(payload) {
      if (payload === null || typeof payload !== "object" && typeof payload !== "function") return false;
      return Object.hasOwn(payload, "presentationMode") || Object.hasOwn(payload, "tablePresentation");
    }
    var sourceBindingModulePromise;
    function getSourceBindingModule() {
      sourceBindingModulePromise ??= Promise.resolve().then(() => (init_office_table_source_binding(), office_table_source_binding_exports));
      return sourceBindingModulePromise;
    }
    var STRUCTURAL_DIGEST_PLACEHOLDER = "0".repeat(64);
    ...
    async function bindSourcesForValidation(bodyMarkdown, sourceFailure, sha256Hex) {
      const { bindOfficeTableSources: bindOfficeTableSources2 } = await getSourceBindingModule();
      try {
        return await bindOfficeTableSources2(
          bodyMarkdown,
          sha256Hex
        );
      } catch {
    ...
    async function validateDescriptorShapeWithSources(bodyMarkdown, descriptor, sourceFailure, sha256Hex) {
      ...
      if (sourceFailure === INVALID_TABLE_PRESENTATION2 && descriptor.tables.length !== sourceTables.tables.length) failInvalid();
      const totalLogicalColumns = sourceTables.tables.reduce(
        (total, source) => total + source.logicalWidth,
        0
      );
      const { getOfficeDescriptorByteLimit: getOfficeDescriptorByteLimit2 } = await getSourceBindingModule();
      const encoder = new TextEncoder();
      ...
      if (descriptorBytes > getOfficeDescriptorByteLimit2(
        bodyBytes,
        sourceTables.tables.length,
        totalLogicalColumns
      )) failInvalid();
      descriptor.tables.forEach((entry, index2) => {
        exactEnumerableKeys(entry, TABLE_KEYS);
        if (!Number.isSafeInteger(entry.tableIndex)) failInvalid();
        if (sourceFailure === INVALID_TABLE_PRESENTATION2 && entry.tableIndex !== index2) failInvalid();
        ...
        if (sourceFailure === INVALID_TABLE_PRESENTATION2 && entry.logicalWidth !== sourceTables.tables[index2].logicalWidth) failInvalid();
        if (entry.columns !== null) {
          ...
          if (sourceFailure === INVALID_TABLE_PRESENTATION2 && entry.columns.length !== entry.logicalWidth) failInvalid();
        }
        ...
        if (entry.borderColor !== null && entry.borderColor !== "default" && (typeof entry.borderColor !== "string" || !COLOR_PATTERN.test(entry.borderColor))) failInvalid();
      });
      return {
    ...
    async function canonicalizeOfficePayloadWithSources(payload, surface, sourceFailure, sha256Hex) {
      ...
      const canOwnFields = payload !== null && (typeof payload === "object" || typeof payload === "function");
      ...
      if (payload.presentationMode !== "office-tables" || typeof payload.content !== "string" || typeof payload.filename !== "string") failInvalid();
      if (surface === "export" && (payload.sourceType !== "document" || payload.sourceFormat !== "markdown" || !["docx", "pdf"].includes(payload.format))) failInvalid();
    ...
    async function canonicalizeOfficePayload2(payload, surface) {
    ...
    async function validateAndBindOfficePayload(payload, surface, sha256Hex) {
      ...
      if (descriptor.markdownSha256 !== sourceTables.markdownSha256 || descriptor.tables.length !== sourceTables.tables.length) failMismatch();
      descriptor.tables.forEach((entry, index2) => {
        const source = sourceTables.tables[index2];
        if (entry.tableIndex !== index2 || entry.sourceSha256 !== source.sourceSha256 || entry.logicalWidth !== source.logicalWidth || entry.columns !== null && entry.columns.length !== source.logicalWidth || entry.titleRow && (source.semanticRows.length < 3 || !source.cellChildCounts[0]?.slice(1).every((count) => count === 0))) failMismatch();
      });
    ...
    module2.exports = {
      INVALID_TABLE_PRESENTATION: INVALID_TABLE_PRESENTATION2,
      TABLE_PRESENTATION_MISMATCH,
      canonicalizeOfficePayload: canonicalizeOfficePayload2,
      hasOfficePresentationFields: hasOfficePresentationFields2,
      presentationError,
...
} = require_office_table_presentation_validation();
