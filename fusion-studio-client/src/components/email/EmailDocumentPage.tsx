/**
 * @module EmailDocumentPage
 * @role Full WYSIWYG document editor for markdown files in email-viewer.
 ...
 *       and renders chrome via EmailDocumentTopbar + EmailDocumentToolbar.
 *
...
interface EmailDocumentPageProps {
...
export function EmailDocumentPage({
  file,
  folderName,
  onBack,
  onOpenFile,
}: EmailDocumentPageProps) {
  ...
  const starredIds = useMemo(
    () => new Set(normalizeViewCollections(rawEmailCollections).starred.map((item) => item.id)),
    [rawEmailCollections]
  );
  ...
  return (
    <div className="rv-email-document-page">
      <EmailDocumentTopbar
        file={file}
      ...
      <div className="rv-email-document-body">
        <EmailDocumentToolbar
          file={file}
        ...
        <div className={`rv-email-document-workspace${sidePanel !== 'none' ? ' rv-email-workspace--drawer-open' : ''}`}>
          <div
            className="rv-email-document-editor"
            style={{
              '--editor-zoom': zoom,
      ...
      <div className="rv-email-document-sidepanel" data-open={sidePanel !== 'none'}>
        {sidePanel === 'files' && (
          <div className="rv-email-document-ribbon-scroll">
            {recentDocs.length === 0 ? (
              <div className="rv-email-document-sidepanel-empty">
