 ...
 *       and zoom controls. All state is owned by EmailDocumentPage.
 */
...
interface EmailDocumentToolbarProps {
...
export function EmailDocumentToolbar({
  file,
  docSettings,
  setDocSettings,
  zoom,
  ...
  return (
    <div className="rv-email-document-toolbar">
      <button
        className="rv-email-document-toolbar-btn"
      ...
      <div className="rv-email-document-toolbar-divider" />

      <select
        className="rv-email-document-toolbar-select"
      ...
      <span className="rv-email-document-toolbar-fontsize">{docSettings.font.size}</span>
      <button
        className="rv-email-document-toolbar-btn"
      ...
      {(['left', 'center', 'right', 'justify'] as const).map((align) => (
        <button
          key={align}
          className={`rv-email-document-toolbar-btn${docSettings.alignment === align ? ' rv-email-toolbar-btn--active' : ''}`}
      ...
      <div className="rv-email-margins-menu" ref={marginsMenuRef}>
        <button
          className={`rv-email-document-toolbar-btn${marginsMenuOpen ? ' rv-email-toolbar-btn--active' : ''}`}
        ...
        {marginsMenuOpen && (
          <div className="rv-email-margins-dropdown">
            {(['top', 'bottom', 'left', 'right'] as const).map((side) => (
              <div key={side} className="rv-email-margins-row">
      ...
      <span className="rv-email-document-toolbar-zoom">{Math.round(zoom * 100)}%</span>
      <button
        className="rv-email-document-toolbar-btn"
