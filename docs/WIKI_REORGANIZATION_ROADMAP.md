...
Active wiki root: `/Users/rccurtrightjr./projects/fs-dev/ai/views/wiki-viewer/Wiki`

Important: `ai/` is ignored by git in this workspace. Wiki edits under
`ai/views/wiki-viewer/Wiki/` are real filesystem edits, but `git status` will
...
`/Users/rccurtrightjr./projects/fs-dev/ai/views/wiki-viewer/Wiki/`

The old generated/mirrored content tree was removed:

`/Users/rccurtrightjr./projects/fs-dev/ai/views/wiki-viewer/content/`
...
- Added frontmatter to all Chat System articles using the contract from:
  `ai/views/wiki-viewer/Wiki/001-Workspaces_And_Views/004-Wiki_View/001-Architecture/006-Frontmatter_Model/PAGE.md`
...
1. `ai/views/wiki-viewer/Wiki/001-Project/008-Workspaces/PAGE.md`
...
2. `ai/views/wiki-viewer/Wiki/001-Project/006-Workspace_Agent_Model/PAGE.md`
...
3. `ai/views/wiki-viewer/Wiki/001-Project/007-Workspace_Index/PAGE.md`
   - Likely belongs under `Workspaces_And_Views/View_Architecture`.
   - Verify current code still uses described index behavior.

4. `ai/views/wiki-viewer/Wiki/001-Project/005-Progressive_Disclosure/PAGE.md`
...
5. `ai/views/wiki-viewer/Wiki/001-Project/012-Browser_Views/`
...
6. `ai/views/wiki-viewer/Wiki/001-Project/013-Background_Agents/PAGE.md`
   - Move under `Automation_And_Agents`.
   - Mark planned/incomplete sections clearly.

7. `ai/views/wiki-viewer/Wiki/001-Project/022-Ticket_Routing/PAGE.md`
   - Move under `Automation_And_Agents/Ticketing`.
   - Note ticketing is partially finished and backend ticketing logic is not
     complete.

8. `ai/views/wiki-viewer/Wiki/001-Project/014-Background_Services_Audit/PAGE.md`
...
9. `ai/views/wiki-viewer/Wiki/001-Project/023-Warmth_Settings/PAGE.md`
   - Audit against current chat runtime decisions.
   - Likely fold into `Chat_System/Runtime_Model` or `Server_And_Runtime`.

10. `ai/views/wiki-viewer/Wiki/001-Project/009-Wiki/`
...
- `ai/views/wiki-viewer/Wiki/001-Project/002-Chat_System/`
- `ai/views/wiki-viewer/Wiki/001-Project/009-Wiki/`
- `ai/views/wiki-viewer/Wiki/003-Enforcement/001-Code_Standards/PAGE.md`
- `ai/views/wiki-viewer/Wiki/003-Enforcement/002-Themes_And_State/PAGE.md`
...
- `find ai/views/wiki-viewer/Wiki -maxdepth 2 -type d | sort` shows the target
  top-level folders.
...
```bash
grep -R "001-Project/002-Chat\\|002-Chat/\\|Coding-CLIs\\|Model_Config\\|Chat_Panel_Architecture\\|Chat_Thread_Lifecycle" -n ai/views/wiki-viewer/Wiki
```

Check all wiki pages have the expected frontmatter once migrated:

```bash
node -e "const fs=require('fs'),path=require('path');function walk(d){return fs.readdirSync(d,{withFileTypes:true}).flatMap(e=>{const p=path.join(d,e.name);return e.isDirectory()?walk(p):(e.name==='PAGE.md'?[p]:[])})}const bad=[];for(const f of walk('ai/views/wiki-viewer/Wiki')){const s=fs.readFileSync(f,'utf8');if(!s.startsWith('---\\nname: '))bad.push(f)};if(bad.length){console.log(bad.join('\\n'));process.exit(1)}console.log('wiki frontmatter ok')"
```

Check no removed folders remain:

```bash
find ai/views/wiki-viewer/Wiki -maxdepth 3 -type d | sort
```

Check active code does not point at old `content/` wiki paths:

```bash
grep -R "wiki-viewer/content\\|views/wiki-viewer/content" -n fusion-studio-client/src fusion-studio-server/lib ai/views/wiki-viewer/Wiki
```

Whitespace check:

```bash
git diff --check -- ai/views/wiki-viewer/Wiki docs/WIKI_REORGANIZATION_ROADMAP.md
```
...
- `ai/views/wiki-viewer/Wiki/PAGE.md`
- `ai/views/wiki-viewer/Wiki/001-Workspaces_And_Views/004-Wiki_View/001-Architecture/006-Frontmatter_Model/PAGE.md`
