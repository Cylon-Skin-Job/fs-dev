...
└── System Files/               # The "special" preloaded workspace
    ...
    └── views/                  # Template sources (mirrors ai/views/)
```
...
### Discovery
Views are discovered by scanning `ai/views/<id>/index.json` in the active workspace.
...
| `settings/layout.json` | Optional layout overrides |
...
- `__panels__` pseudo-panel resolves to `ai/views/`
...
**View:** `system-viewer` (iframe view inside `System Files/ai/views/system-viewer/`)
...
- `System Files/ai/views/index.json`
