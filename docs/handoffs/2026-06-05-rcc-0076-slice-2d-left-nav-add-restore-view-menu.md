...
- Panel discovery prefers `ai/system/workspace/views.json`.
- Hidden views remain in `ai/system/workspace/views.json` with `enabled: false`.
...
- `Restore View`: show hidden installed views from `ai/system/workspace/views.json` and set `enabled: true`.
- `Add View`: show available manifest templates not already installed in the workspace, copy the selected template into `ai/views/<view-id>`, and append a new enabled registry entry.
...
- Selecting a hidden view sets `enabled: true` in `ai/system/workspace/views.json` without copying files.
...
- Selecting a template copies its template folder into the active workspace's `ai/views/<view-id>` and appends a registry entry.
...
- Add View writes `viewPath: "ai/views/<view-id>"`.
...
- Do not edit existing per-view `ai/views/<view-id>/index.json` except by copying a brand-new template folder for an added view.
...
- Reject if destination `ai/views/<templateId>` already exists unexpectedly.
- Resolve template source under `System Source Files/view-templates`.
- Resolve destination under active workspace `ai/views`.
...
- Do not edit legacy `ai/views/index.json`.
...
```bash
rg -n "ai/views/index.json" fusion-studio-server/lib/views fusion-studio-server/lib/ws fusion-studio-client/src
```
...
8. Confirm `ai/views/<template-id>/` was created from the template.
9. Confirm `ai/system/workspace/views.json` has a new enabled entry.
10. Confirm no legacy `ai/views/index.json` edit occurred.
...
Right-clicking empty left-nav space lets users restore hidden installed views and add new manifest-backed views to the active workspace, with all live state persisted through `ai/system/workspace/views.json` and no folder deletion or legacy registry writes.
