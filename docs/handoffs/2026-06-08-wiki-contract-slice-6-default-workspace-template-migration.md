...
- Active wiki viewer reads `ai/views/wiki-viewer/Wiki`.
...
- `Fusion-Home/ai/views/wiki-viewer` has `content/`, `content.json`, `index.json`, `settings/`, `api.json`, `chat/`.
- `Fusion-Home/workspace-templates/*/ai/views/wiki-viewer` have similar `content/` structures.
- `solobooks/ai/views/wiki-viewer` has a fuller legacy wiki package with `content/`, `PROMPT.md`, `SPEC.md`, `WORKFLOW.md`, `runs/`, etc.
- `media-editor/ai/views/wiki-viewer` is skeletal and currently has only `content.json` and `index.json`.
...
```text
workspace-templates/fusion-home/ai/views/wiki-viewer/Wiki/
```
...
`media-editor/ai/views/wiki-viewer` is skeletal. Create a minimal canonical `Wiki/` tree that matches the Media Studio template enough for the viewer to render.
