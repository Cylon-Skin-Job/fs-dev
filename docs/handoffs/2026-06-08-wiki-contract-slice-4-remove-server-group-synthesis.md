...
The active wiki viewer now discovers folders under `ai/views/wiki-viewer/Wiki` using `file_tree_request` and loads selected `PAGE.md` files using `file_content_request`. The server should no longer synthesize old `index.json` group payloads for `wiki-viewer`.
