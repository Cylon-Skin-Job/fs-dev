...
4. Slice 2b: migrate live view registry to `ai/system/workspace/views.json`.
...
- Each selected view template creates its own data under the new workspace's `ai/views`.
...
The live view/workspace metadata should be file-based and live in the actual workspace's `ai/system/`, not in SQLite.
...
ai/system/state/workspace.json
...
- `ai/system/workspace/views.json` should hold the workspace's view list, order, icon, and any membership flags.
- `ai/system/state/workspace.json` can hold active/ephemeral workspace UI state if needed.
- `ai/views/` should stay focused on the actual view folders/content, not the workspace registry.

### Slice 2b: Migrate live view registry out of `ai/views/index.json`

Move the live workspace view registry to `ai/system/workspace/views.json` so `ai/views/` only contains view content folders.

Proposed `ai/system/workspace/views.json` schema:
...
```json
{
  ...
  "views": [
    ...
    },
    {
      ...
      "enabled": true,
      "source": "default",
      "viewPath": "ai/views/issues-viewer"
    }
  ]
}
```
...
- `id`: stable view id, matching the folder name under `ai/views/`.
...
- Future right-click rename/icon/reorder actions should update `ai/system/workspace/views.json`.
- Future hide/show actions should update `enabled` in `ai/system/workspace/views.json` without deleting folders.
- The view folder's own `ai/views/<viewId>/index.json` can continue to describe the view implementation itself, but not global workspace ordering.
- If both old `ai/views/index.json` and new `ai/system/workspace/views.json` exist during migration, the new `ai/system/workspace/views.json` should win.
...
- The workspace's view list, icons, and ranks are loaded from `ai/system/workspace/views.json`.
- The legacy `ai/views/index.json` registry is no longer the source of truth.
...
- The context menu supports hiding the view by setting `enabled: false` in `ai/system/workspace/views.json`.
...
- `Restore View` lists views with `enabled: false` from `ai/system/workspace/views.json`.
...
- For each workspace, inspect its `ai/views/` folders and existing view `index.json` files.
- Record each view's current folder name, declared `id`, label, icon, rank, content config, and layout settings.
- Generate or update `ai/system/workspace/views.json` from the existing workspace data.
...
5. The selected views' `templatePath` values are used to populate `ai/views`.
