...
├── views-watcher.js        ← new — watches ai/views/, emits UEB events
...
One job: watch `<workspace>/ai/views/` and emit UEB events when views are added,
removed, or changed.
...
- [ ] Add a new folder to `ai/views/` → `views:added` emitted on event bus
- [ ] Remove a folder from `ai/views/` → `views:removed` emitted
