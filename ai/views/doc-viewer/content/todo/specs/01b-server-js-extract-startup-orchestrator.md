...
1. `ai/views/doc-viewer/content/todo/specs/01-server-js-CONTEXT-FORWARD.md` — the resume-from-compact doc for all of SPEC-01. Read the gotchas section, especially **#4 Startup sequence has hard order dependencies**.
2. `ai/views/doc-viewer/content/todo/specs/01-server-js-decomposition.md` — the original decomposition spec.
3. `ai/views/doc-viewer/content/todo/specs/01a-server-js-extract-file-explorer.md` — the previous extraction spec (completed). Read this for the factory-pattern template and the verification workflow. 01b follows the same shape.
