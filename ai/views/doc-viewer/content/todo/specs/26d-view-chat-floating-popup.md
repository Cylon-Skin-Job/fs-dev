...
2. **`ai/views/doc-viewer/content/todo/specs/26c-2-dual-chat-slider-persistence.md`** — understand the existing `viewStates` persistence layer you're extending.
...
4. **State file audit.** `cat ai/views/file-viewer/state/<username>.json` after using the popup. Confirm `popup` sub-object is present alongside the existing `collapsed` and `widths`.
