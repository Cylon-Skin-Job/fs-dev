...
1. `ai/views/doc-viewer/content/todo/specs/01-server-js-CONTEXT-FORWARD.md` — gotchas, especially **#1 Session closure scope** and **#3 `global.__agentWireSessions`**.
2. `ai/views/doc-viewer/content/todo/specs/01a–d` — prior extraction specs. Read 01c for the per-connection factory pattern and 01d for the "create router first, then lifecycle" ordering rule.
3. `ai/views/doc-viewer/content/todo/REFACTOR-LOG.md` — read the SPEC-01c and SPEC-01d entries, particularly the `agentWireSessions` surprise.
