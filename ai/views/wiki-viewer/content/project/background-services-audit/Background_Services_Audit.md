...
| Workspace controller | `lib/startup.js` -> `workspaceController.start()` | Filesystem paths from workspace registry | Registered repos still exist and have `ai/views/index.json` | Startup rejects on unexpected DB/FS errors | Keep required. Consider documenting that missing registered workspaces are culled on launch. |
...
| CLI config ensure | `lib/startup.js` -> `ensureWorkspaceFile()` | Active workspace `ai/system/config/cli.json` | Workspace writable | Promise catch logs warning | Keep guarded. |
...
| Agent trigger loader | `_startPipeline()` -> `loadTriggers()` | Recursive scan of `ai/views/agents-viewer`, `ai/views`, `ai/components` | `registry.json` exists for agent triggers | Outer startup catch logs failure | Keep, but make trigger automation workspace-config opt-in. |
...
| Panel discovery | `ws.onopen` -> `loadAllPanels()` | Workspace filesystem via WS | `ai/views` and optional `ai/apps` readable | Per-file timeouts/catches | Keep, but discovery probes many files per panel. Cache or batch if startup is slow. |
| Shared style loading | `useSharedWorkspaceStyles()` | Workspace style files via WS | `ai/system/styles` optional | Catch logs error | Keep. Server already includes styles in `workspace:init`, so avoid redundant refetch where possible. |
...
| Recent docs | `OfficeDocumentPage` mount and document open | Server SQLite, file content preloads | Workspace ID and office files exist | Handler catches; content requests catch | Keep request-driven. |
