...
1. `ai/<machine>/Captures/003-TODO/specs/01-server-js-CONTEXT-FORWARD.md` — SPEC-01 resume doc. Read gotchas **#1 Session closure scope**, **#5 checkSettingsBounce**.
2. `ai/<machine>/Captures/003-TODO/specs/01-COORDINATION-with-SPEC-23.md` — the coordination doc that mandates this combined extraction.
3. `ai/<machine>/Captures/003-TODO/specs/23-CONTEXT-FORWARD.md` — SPEC-23 master orientation. Read "Critical invariants" (sections 3 and 4 especially).
4. `ai/<machine>/Captures/003-TODO/specs/23a-bus-consolidation-wire-broadcaster.md` — the SPEC-23 sub-spec this spec absorbs. Read the "Gotchas" section and the "Silent fail risks" table.
5. `ai/<machine>/Captures/003-TODO/specs/01a-`, `01b-`, `01c-server-js-extract-*.md` — all three completed specs. Read for the factory-pattern + verification workflow.
6. `ai/<machine>/Captures/003-TODO/REFACTOR-LOG.md` — observations across prior extractions. Read the 01c section specifically (`agentWireSessions` surprise, hoisting dependency, per-connection factory pattern).
...
**Transcribe the current handleWireMessage body character-for-character into the switch statement**, with the single exceptions listed above (delete the 7 chat ws.send calls; add the bounce-path emit). Do not "improve" any case. Do not consolidate the `workspace: 'file-viewer'` hardcoding (it's a pre-existing bug; out of scope — see Gotcha #8).
...
### 8. `workspace: 'file-viewer'` hardcoding is a pre-existing bug — DO NOT FIX

Every chat emit in the current handleWireMessage hardcodes `workspace: 'file-viewer'`:

```js
emit('chat:content', { workspace: 'file-viewer', ... });
```

The actual panel/view name depends on the connection's current panel (which can be `doc-viewer`, `agents-viewer`, etc., not always `file-viewer`). This is a latent bug — anything filtering bus events by workspace would incorrectly attribute all events to `file-viewer`.
...
14. **Settings bounce test.** Ask the AI (in chat) to write to a file inside `ai/<machine>/System/styles/` or `ai/<machine>/Views/*/settings/`. The bounce should fire — the client should see a `tool_result` with `isError: true` and the bounce message as output. Confirm by watching the chat UI.
...
- **Do not** fix the `workspace: 'file-viewer'` hardcoding. See Gotcha #8.
...
Everything else preserved exactly:
  ...
  - workspace: 'file-viewer' hardcoding (pre-existing bug, out of scope)
