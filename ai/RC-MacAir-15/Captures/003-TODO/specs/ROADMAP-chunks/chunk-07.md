...
1. In a single global file: `ai/<machine>/System/styles/tints.css`
...
**Step 2: Write `ai/<machine>/System/styles/tints.css`**
...
5. Verify no orphaned `.rv-panel[data-tint-*]` rules remain in any CSS file:
   ```bash
   grep -r "data-tint-" open-robin-client/src/components/ ai/<machine>/Views/
   ```
   The only results should be in `ai/<machine>/System/styles/tints.css`.
