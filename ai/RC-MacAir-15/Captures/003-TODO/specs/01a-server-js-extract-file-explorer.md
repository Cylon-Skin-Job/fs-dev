...
### 3. Preserve the `panel || 'file-viewer'` defaults

All three handlers default `panel` to `'file-viewer'` when `msg.panel` is falsy. Don't change this default. Don't "improve" it to the current actual panel or to null. It's a historical default.
...
8. Open a file explorer panel in the UI (doc-viewer has one). It should populate with folders. Click into a folder — the tree should expand. Click on a file — content should load in the viewer.
