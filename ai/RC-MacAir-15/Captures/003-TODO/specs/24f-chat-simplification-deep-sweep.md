...
Five chat-enabled views have `settings.layout` in their `index.json` set to a value that hides the thread sidebar (`full` or `chat-content`). Flip them all to `sidebar-chat-content` so the chat sidebar actually renders for those panels, matching the known-good `file-viewer` experience. **This is a stopgap** — a future spec (SPEC-26) replaces the three-layout switch with a collapsible drawer, at which point these JSON values get repurposed or deleted. For now, the flip is what lets the user interact with the SPEC-24a-24e chat work on any panel beyond `file-viewer`.
...
- **Live validation on a non-file-viewer panel is finally possible.** Open issues-viewer, create a thread, see it appear in the sidebar.

**The 24x series is complete after this spec lands.**

**You are not touching:**
- `panels.ts:152-159` file-viewer special-case layout path — load-bearing prototype code, not dead
...
- `hooks/useFileTree.ts` file-viewer gating — correct behavior
- `hooks/usePanelWorkspaceStyles.ts` file-viewer gating — correct behavior
- `state/panelStore.ts:100` `currentPanel: 'file-viewer'` default — cosmetic
...
- `ai/<machine>/Agents/` or anything about agent prompts/workflows — saved feedback says don't audit
...
2. **`ai/<machine>/Captures/003-TODO/specs/24-chat-simplification.md`** — parent spec.
...
```bash
for v in agents-viewer browser-viewer doc-viewer file-viewer docs-viewer email-viewer issues-viewer library-viewer wiki-viewer; do
  f=ai/<machine>/Views/$v/index.json
  c=ai/<machine>/Views/$v/content.json
  if [ -f "$f" ]; then
...
doc-viewer     layout=sidebar-chat-content   chat=yes   (OK)
file-viewer        layout=sidebar-chat-content   chat=yes   (OK)
...
For each of the five views listed in the pre-flight audit, edit `ai/<machine>/Views/<view>/index.json` and change the `settings.layout` field to `"sidebar-chat-content"`.

Example for `issues-viewer`:

Current (`ai/<machine>/Issues/index.json`):
```json
{
  ...
  "settings": {
...
**Important:** do NOT touch `file-viewer`, `docs-viewer`, `email-viewer`, `doc-viewer` — those already have `sidebar-chat-content` and must not be edited.
...
for v in agents-viewer browser-viewer doc-viewer file-viewer docs-viewer email-viewer issues-viewer library-viewer wiki-viewer; do
  layout=$(python3 -c "import json; print(json.load(open('ai/<machine>/Views/$v/index.json')).get('settings',{}).get('layout','(none)'))")
...
4. **Switch between panels** and verify the thread lists stay scoped correctly. Creating a thread in issues-viewer should only populate issues-viewer's sidebar. Switching to file-viewer should show file-viewer's threads, not issues-viewer's.

5. **Click the "copy link" menu action** on a thread in file-viewer's sidebar (or any panel). Verify it still copies the file path to the clipboard (this is the preserved `thread:link` handler from Item 2).
...
- **Do not** touch `file-viewer`, `docs-viewer`, `email-viewer`, or `doc-viewer` index.json — they already have the correct layout.
...
- **Do not** touch `panels.ts:152-159` file-viewer layout path special case — it's load-bearing prototype code for the per-view CSS architecture, not dead.
...
5. **Live validation evidence.**
   ...
   - Screenshot or description of each non-file-viewer panel showing the sidebar (at minimum: issues-viewer, wiki-viewer)
