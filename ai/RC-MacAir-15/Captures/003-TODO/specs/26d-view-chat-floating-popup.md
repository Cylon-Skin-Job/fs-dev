...
Same file (`ai/<machine>/Views/<view>/state/<username>.json`), same `state:get/set` wire messages, same server module. Just a richer object. The server's `writeViewStatePatch` already merges partial patches, so sending `{ popup: { x: 500 } }` correctly deep-merges.
...
When the user closes the popup in file-viewer and switches to issues-viewer, then comes back to file-viewer, the popup is still closed (they have to click FAB). But if they left the popup OPEN and switch away, returning to file-viewer finds it open again. This is because `popup.open` is stored in `viewStates[panel]` and persisted via the JSON file.
...
2. **`ai/<machine>/Captures/003-TODO/specs/26c-2-dual-chat-slider-persistence.md`** — understand the existing `viewStates` persistence layer you're extending.
...
**FAB presence:**
1. Chat-enabled view (file-viewer) → FAB visible in bottom-right.
...
14. Create a view thread in file-viewer. Close the popup.
...
17. Switch back to file-viewer. Click FAB → file-viewer's thread (not issues-viewer's).
...
20. Refresh the browser. Open file-viewer. State from the JSON file loads — position and size preserved. Open state depends on whether it was open when the JSON was last written.
...
JSON file at ai/<machine>/Views/<view>/state/<username>.json:
...
4. **State file audit.** `cat ai/<machine>/Data/File-Viewer/state/<username>.json` after using the popup. Confirm `popup` sub-object is present alongside the existing `collapsed` and `widths`.
