...
| `chat` | (stub) Chat thread .md files under ai/<machine>/Views/*/chat/threads/ | no |
...
1. **`ai/<machine>/Wiki/enforcement/code-standards/PAGE.md`** — the house rules. Pay attention to "one job per file", "delete don't deprecate", and "no premature abstractions".
2. **`ai/<machine>/Captures/003-TODO/specs/24-chat-simplification.md`** — SPEC-24 parent. Understand that SPEC-25 is a precursor for 24b.
...
If the issues-viewer has any ticket files in `ai/<machine>/Issues/tickets/`:

```bash
# List what's there
ls ai/<machine>/Issues/tickets/*.md 2>/dev/null
...
const dir = 'ai/<machine>/Issues/tickets';
