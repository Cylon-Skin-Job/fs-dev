...
2. **`ai/<machine>/Captures/003-TODO/specs/24-chat-simplification.md`** — parent of the 24x series (note that 24x is DONE as of commit 8e265cc).
...
```bash
sqlite3 /Users/rccurtrightjr./projects/open-robin/ai/<machine>/System/robin.db ".schema threads"
```
...
```bash
sqlite3 /Users/rccurtrightjr./projects/open-robin/ai/<machine>/System/robin.db "SELECT COUNT(*) FROM exchanges WHERE thread_id NOT IN (SELECT thread_id FROM threads);"
```
...
sqlite3 ai/<machine>/System/robin.db "SELECT COUNT(*) FROM threads;" > /tmp/26a-pre-threads.txt
sqlite3 ai/<machine>/System/robin.db "SELECT COUNT(*) FROM exchanges;" > /tmp/26a-pre-exchanges.txt
sqlite3 ai/<machine>/System/robin.db "SELECT COUNT(*) FROM exchanges WHERE thread_id NOT IN (SELECT thread_id FROM threads);" > /tmp/26a-pre-orphans.txt
...
try { new ThreadIndex('proj', 'view', 'file-viewer'); console.log('PASS: valid construction'); } catch (e) { console.log('FAIL:', e.message); }
try { new ThreadIndex(); console.log('FAIL: should have thrown'); } catch (e) { console.log('PASS: empty rejected'); }
try { new ThreadIndex('proj', 'bogus', 'file-viewer'); console.log('FAIL: bogus scope accepted'); } catch (e) { console.log('PASS: bogus scope rejected'); }
...
try { new ThreadManager('file-viewer', { projectRoot: '/tmp/fake-project' }); console.log('PASS: construction with projectRoot'); } catch (e) { console.log('FAIL:', e.message); }
try { new ThreadManager('file-viewer', {}); console.log('FAIL: no projectRoot accepted'); } catch (e) { console.log('PASS: missing projectRoot rejected'); }
"
# Expected: both PASS
...
# Migration log
sqlite3 /Users/rccurtrightjr./projects/open-robin/ai/<machine>/System/robin.db "SELECT name, batch FROM knex_migrations ORDER BY batch, name;"
...
2. **Create a thread on file-viewer** through the normal UI flow (harness picker → Kimi → send a message).

3. **Check SQLite — the new row has the 26a columns populated.**
   ```bash
   sqlite3 /Users/rccurtrightjr./projects/open-robin/ai/<machine>/System/robin.db "SELECT thread_id, project_id, scope, view_id, name, message_count FROM threads;"
   ```
   Expected:
...
5. **Sidebar scoping still works.** Switch between file-viewer and issues-viewer; each shows only its own threads. This should be identical behavior to before — 26a doesn't change the routing, only the underlying column names.
...
| Cross-panel leak (file-viewer sees issues-viewer threads) | view_id column populated correctly but list() isn't filtering | Re-check `list()` — `scope='view'` branch must add `.where('view_id', this.viewId)` |
...
Live-validated: create/rename/delete on file-viewer and
...
5. **Live validation evidence.**
   ...
   - SQLite row contents after creating one thread on file-viewer and one on issues-viewer — confirm project_id, scope, view_id fields are populated correctly
