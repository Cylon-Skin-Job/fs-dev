...
2. **`ai/<machine>/Captures/003-TODO/specs/26a-dual-chat-data-model.md`** — what 26a delivered.
3. **`ai/<machine>/Captures/003-TODO/specs/26b-dual-chat-routing-layer.md`** — what 26b delivered, especially the wire protocol scope field.
...
case 'thread:created':
  if (msg.thread && msg.threadId) {
    ...
    // Optional: loadRootTree() for file-viewer only — leave as-is for compat.
  }
  return true;
...
### 8. CSS — `ai/<machine>/System/styles/styles/views.css`
...
Find them:
```bash
grep -l '"layout"' /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/*/index.json
```
...
1. **Code-viewer loads with 5 columns visible.** Left sidebar (labeled "Project" or similar), left chat pane, content (file tree), right chat pane, right sidebar (labeled "file-viewer" or similar).
...
3. **Create a project thread.** Click "New Thread" in the LEFT sidebar. Pick a harness. Send a message.
   ...
   - Filesystem: file lands at `ai/<machine>/Views/chat/threads/<user>/<id>.md`
...
4. **Create a view thread.** Click "New Thread" in the RIGHT sidebar. Pick a harness. Send a message.
   ...
   - SQLite shows second row with `scope=view`, `view_id=file-viewer`
   - Filesystem: file lands at `ai/<machine>/Data/Chatlogs/threads/<user>/<id>.md`
...
7. **Switch back to file-viewer.** Verify:
   - Right sidebar shows the file-viewer view thread (NOT the issues-viewer view thread)
...
find /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/chat/threads -type f -name '*.md'
echo "=== view threads ==="
find /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/*/chat/threads -type f -name '*.md' 2>/dev/null | grep -v '^/Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/chat/'
```
...
Live-validated across file-viewer, issues-viewer, wiki-viewer:
  ...
  - Creating a project thread → SQLite scope=project, view_id=NULL,
    file at ai/<machine>/Views/chat/threads/<user>/<id>.md
  - Creating a view thread → SQLite scope=view, view_id=<panel>,
    file at ai/<machine>/Views/<view>/chat/threads/<user>/<id>.md
