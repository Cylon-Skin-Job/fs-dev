...
- Global tint CSS lives in `ai/<machine>/Views/tints.css` (or injected into `themes.css`)
...
| File | Change |
|------|--------|
| `ai/<machine>/Views/tints.css` | **NEW** — global tint rules for all views |
...
| `ai/<machine>/Data/File-Viewer/styles/layout.css` | Remove `.rv-panel[data-tint-*]` rules |
| `ai/<machine>/System/styles/views.css` | Remove `.rv-panel[data-tint-*]` rules (chat borders, thread borders) |

### Data Migration

| File | Change |
|------|--------|
| `ai/<machine>/System/styles/state.json` | Move `tints` object out into `ai/<machine>/System/styles/themes.json` active theme |
| `ai/<machine>/System/styles/themes.json` | Add `tints` block to every theme entry; default all false |
...
```css
/* ai/<machine>/Views/tints.css — static file, reads from :root variables set by theme */
...
- Create `ai/<machine>/Views/tints.css` with all `body[data-tint-*]` rules
...
- Should `tints.css` be a static file (loaded once) or generated into `themes.css` (regenerated on theme switch)?
  - **Recommendation:** Static file at `ai/<machine>/Views/tints.css`. The selectors don't change; only the `data-tint-*` attributes on `<body>` change. Static file = simpler, no regeneration cost.
