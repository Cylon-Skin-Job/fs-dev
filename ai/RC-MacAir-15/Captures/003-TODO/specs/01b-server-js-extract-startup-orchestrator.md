...
1. `ai/<machine>/Captures/003-TODO/specs/01-server-js-CONTEXT-FORWARD.md` — the resume-from-compact doc for all of SPEC-01. Read the gotchas section, especially **#4 Startup sequence has hard order dependencies**.
2. `ai/<machine>/Captures/003-TODO/specs/01-server-js-decomposition.md` — the original decomposition spec.
3. `ai/<machine>/Captures/003-TODO/specs/01a-server-js-extract-file-explorer.md` — the previous extraction spec (completed). Read this for the factory-pattern template and the verification workflow. 01b follows the same shape.
...
function _startPipeline({ sessions, getDefaultProjectRoot, AI_PANELS_PATH }) {
  // Wiki hooks — watches ai/<machine>/Wiki/
...
7. `tail -30 open-robin-server/server-live.log` should show the expected startup log lines in the expected order:
   ...
   [Server] Thread storage: /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views
   ```
