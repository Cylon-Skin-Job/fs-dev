...
`views.css` — the chat/thread chrome that every view depends on — is currently baked into the JS bundle via a Vite `@views` alias that points to open-robin's `ai/<machine>/Views/`. This means:
...
Three CSS files under `ai/<machine>/System/styles/styles/` apply to every view:
...
```
ai/<machine>/Views/<viewer>/settings/styles/layout.css
```
...
```ts
'@views': path.resolve(__dirname, '../ai/<machine>/Views'),
```
...
/**
 * Load optional per-view layout CSS from ai/<machine>/Views/<panelId>/settings/styles/layout.css.
...
2. **Open open-robin workspace.** Chat/thread chrome renders correctly (loaded at runtime from `ai/<machine>/System/styles/styles/views.css`, not bundled).
