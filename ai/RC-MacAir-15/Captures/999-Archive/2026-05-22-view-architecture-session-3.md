...
| B | Filesystem Restructure (`ai/<machine>/System/styles/` → `ai/<machine>/System/`) | RCC-0083 |
| C | Custom Protocol `fusion-studio://` | RCC-0084 |

All five are filed as closed tickets in `ai/<machine>/Issues/done/`.
...
3. **Force-reload CSS bug** — `workspace:init` in `server.js` (line 516) was pre-reading CSS from `ai/<machine>/System/styles/` (old path → empty). Client called `injectWorkspaceStyles([])` which stripped all CSS. Fix: corrected to `ai/<machine>/System/styles`.

4. **Four Chunk B path misses** — `themes-service.js` (2), `workspace-broadcaster.js` (1), `server.js` `/api/view-config` (1) — all still referenced `ai/<machine>/System/styles/`. Found via grep audit after testing.
...
**Spec location:** needs to be written — `ai/<machine>/Views/doc-viewer/specs/chunks/chunk-D-view-loader-iframe.md`
...
> Delete `lib/watch/views-watcher.js`, replace with `file:changed` UEB listener filtering `ai/<machine>/Views/` paths. No view ever gets its own backend watcher.
...
- Some server listener replacing it (UEB `file:changed` filter on `ai/<machine>/Views/`)
...
| Chunk D spec (to write) | `ai/<machine>/Views/doc-viewer/specs/chunks/chunk-D-view-loader-iframe.md` |
| Chunk C spec (complete) | `ai/<machine>/Views/doc-viewer/specs/chunks/chunk-C-custom-protocol.md` |
...
| Code Standards | `System Files/ai/<machine>/Wiki/enforcement/code-standards/PAGE.md` |
