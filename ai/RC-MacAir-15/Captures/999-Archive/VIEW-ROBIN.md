...
### Triggers
Aggregated view of every `TRIGGERS.md` found in `ai/<machine>/Views/` and `/system/`. Dashboard, not editor. Each entry links back to its source file. No validation baked in — a background agent audits if needed.
