...
`ai/<machine>/Views/<panel>/chat/threads/<user>/<threadId>.md`
to a unified location at
`ai/<machine>/Views/chat/threads/<user>/<threadId>.md`.
...
**Pre-prod wipe required.** Existing thread files under every `ai/<machine>/Views/<panel>/chat/threads/` directory must be deleted during validation. Per parent SPEC-24, these are disposable test data. SQLite `threads` table also wiped. No migration of old files to the new location — we start fresh.
...
1. **`ai/<machine>/Wiki/enforcement/code-standards/PAGE.md`** — house rules. Re-read "delete don't deprecate" and "no premature abstractions".
2. **`ai/<machine>/Captures/003-TODO/specs/24-chat-simplification.md`** — parent spec for the 24x series.
...
```bash
find /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/*/chat/threads -type f -name '*.md' 2>/dev/null
sqlite3 /Users/rccurtrightjr./projects/open-robin/ai/<machine>/System/robin.db "SELECT COUNT(*) FROM threads;"
```
...
```js
   ...
   *
   * All chats live at ai/<machine>/Views/chat/threads/<username>/ regardless of
 ...
 * under ai/<machine>/Views/chat/threads/<user>/, so there's no per-panel path to
 ...
 * at ai/<machine>/Views/chat/ — callers no longer need a per-panel path. The
...
```js
  ...
  // to avoid cross-contamination (e.g., issues-viewer seeing file-viewer threads).
  // Only set up threads if the default view has chat.
  // panelId = view name ('file-viewer'), panelPath = absolute chat folder path
  const defaultChatConfig = views.resolveChatConfig(projectRoot, 'file-viewer');
  if (defaultChatConfig) {
    // panelId = view name ('file-viewer'), panelPath = full chat folder path
...
```js
  ...
  // unified at ai/<machine>/Views/chat/threads/<user>/, no panelPath needed).
  const defaultChatConfig = views.resolveChatConfig(projectRoot, 'file-viewer');
  if (defaultChatConfig) {
    ThreadWebSocketHandler.setPanel(ws, 'file-viewer', {
      projectRoot,
...
const expected = '/tmp/fakeproject/ai/<machine>/Views/chat/threads/';
...
node -e "
const { ThreadManager } = require('./lib/thread/ThreadManager');
const m1 = new ThreadManager('file-viewer', { projectRoot: '/tmp/x' });
...
if (m1._getViewsDir() === m2._getViewsDir() && m2._getViewsDir() === m3._getViewsDir()) {
  ...
  console.log('file-viewer:', m1._getViewsDir());
...
const config = views.resolveChatConfig('/Users/rccurtrightjr./projects/open-robin', 'file-viewer');
if (!config) { console.log('SKIP: no chat for file-viewer (unexpected)'); process.exit(0); }
...
# Wipe SQLite threads
sqlite3 /Users/rccurtrightjr./projects/open-robin/ai/<machine>/System/robin.db "DELETE FROM threads;"

# Wipe old per-panel thread files
find /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/*/chat/threads -type f -name '*.md' -delete 2>/dev/null

# Also nuke any empty user-subfolders left behind
find /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/*/chat/threads -mindepth 1 -type d -empty -delete 2>/dev/null

# Remove the pre-existing unified location if it has stale data from your editing
rm -rf /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/chat/threads 2>/dev/null

# Verify clean slate
find /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views -path '*/chat/threads/*' -name '*.md' 2>/dev/null
sqlite3 /Users/rccurtrightjr./projects/open-robin/ai/<machine>/System/robin.db "SELECT COUNT(*) FROM threads;"
...
2. **Create a new thread via the UI** on any chat-enabled panel (file-viewer, issues-viewer, etc.).
...
3. **Verify the file lands in the new unified location.**
   ...
   `/Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/chat/threads/<username>/2026-04-09T14-30-22-123.md`
...
5. **Create a second thread from a DIFFERENT panel.** Switch panels, create another thread. Verify:
   - Both files land in the same `ai/<machine>/Views/chat/threads/<user>/` directory.
...
| Files land in `ai/<machine>/Views/<panel>/chat/threads/` instead of unified | `_getViewsDir()` fallback branch not deleted | Re-check step 1b |
...
```
SPEC-24c: unify chat storage at ai/<machine>/Views/chat/threads/<user>/<id>.md

Relocates thread markdown files from per-panel storage
  ai/<machine>/Views/<panel>/chat/threads/<user>/<id>.md
to a single unified location
  ai/<machine>/Views/chat/threads/<user>/<id>.md
...
6. Pre-prod wipe — existing thread files under every
   ai/<machine>/Views/*/chat/threads/ directory deleted, SQLite threads table
...
4. **Live validation evidence.**
   ...
   - Confirmation that `find ai/<machine>/Views/*/chat/threads -type f` returned zero after creating threads (old locations stay empty).
   - Confirmation that two threads from different panels both landed in the same `ai/<machine>/Views/chat/threads/<user>/` directory.
