...
| `AI_PANELS_PATH` param | Compute from `getProjectRoot()` + `'ai/<machine>/Views'` at call time |
...
Robin handlers use `getDefaultProjectRoot()` to resolve `ai/<machine>/System/styles/themes.css` and the settings directory. Replace with `getProjectRoot`:
