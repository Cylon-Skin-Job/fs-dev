...
**Supersedes:** Per-view state paths from SPEC-26c-2 (`ai/<machine>/Views/<view>/state/<username>.json`).
**Related memories:**
...
- Drop an `ai/<machine>/Views/<view>/state/state.json` file containing only the keys you want pinned for that view. Those keys win; everything else falls through to the workspace default.
- The mechanic is identical to how a per-view stylesheet in `ai/<machine>/Views/<view>/settings/styles/<same-name>.css` will override a workspace stylesheet of the same name (future work).
...
- **No changes to `layout.json`.** `ai/<machine>/Views/<view>/styles/layout.json` remains per-view declared structural intent (chat position, declared defaults) and continues to seed the resolver when no persisted state exists.
...
```
ai/<machine>/System/styles/state.json                ← workspace default (source of truth)
ai/<machine>/Views/<view>/state/state.json         ← optional per-view override (subset of keys)
```
...
**Path:** `ai/<machine>/System/styles/state.json`
...
**Path:** `ai/<machine>/Views/<view>/state/state.json`
...
- `ai/<machine>/Views/<view>/state/<username>.json` — entire `<view>/state/` directory.
...
```jsonc
// ai/<machine>/System/styles/state.json — full shape (all keys present)
{
  "widths": {
    "leftSidebar":     220,
    "leftChat":        320,
...
```jsonc
// ai/<machine>/Views/<wiki-view-folder>/styles/state.json — example override
...
**Fallback:** if neither file exists, the resolver creates `ai/<machine>/System/styles/state.json` with hardcoded defaults (see §5 shape) and returns those. First-run behavior.
...
- The `layoutConfig` type (`panels.ts:40-46`) is unrelated; keep as-is for now. Unrelated follow-up: decide whether to delete it or wire it to seed `ai/<machine>/System/styles/state.json` on first launch (§6 fallback). My recommendation: seed from `layoutConfig` at first launch if it's present, otherwise hardcoded defaults. This gives per-view `layout.json` a real job during bootstrap without resurrecting the "ignore the override" bug.
...
**Inputs:** current `ai/<machine>/Views/<view>/state/<username>.json` files. The candidates today:

```
ai/<machine>/Agents/state/rccurtrightjr..json
ai/<machine>/Views/capture-viewer/state/rccurtrightjr..json
ai/<machine>/Views/code-viewer/state/rccurtrightjr..json
ai/<machine>/Issues/state/rccurtrightjr..json
ai/<machine>/Views/<wiki-view-folder>/state/rccurtrightjr..json
```
...
3. Write the seed into `ai/<machine>/System/styles/state.json` (create folder).
4. For each other view, diff its keys against the seed:
   - If a key's value matches the seed → drop it (no override needed).
   - If it differs → include it in a per-view override at `ai/<machine>/Views/<view>/state/state.json`.
   - If the per-view override would be empty `{}` → do not create the file.
5. Delete old `ai/<machine>/Views/<view>/state/` directories.
6. Log everything to stdout so the diff is visible.

**Expected outcome (based on current state dumps I inspected):**

- `ai/<machine>/System/styles/state.json` seeded from code-viewer.
- `ai/<machine>/Views/<wiki-view-folder>/styles/state.json` → `{ "collapsed": { "leftSidebar": true } }`.
- `ai/<machine>/Issues/state/state.json` → `{ "collapsed": { "leftSidebar": true } }`.
...
| `ai/<machine>/System/styles/state.json`                                 | Created by migration script. |
| `ai/<machine>/Views/<view>/state/state.json`                          | Created by migration script for views with diverging keys (see §10). |
| `ai/<machine>/Views/<view>/state/rccurtrightjr..json` (all views)        | Deleted by migration script. |
...
4. **Migration script** — run once manually, inspect output, commit the resulting `ai/<machine>/System/styles/state.json` and any per-view overrides.
...
7. **Document the pattern** — add a wiki page under `ai/<machine>/Wiki/enforcement/` (or similar) so future AI implementers know the override-by-duplication pattern applies to `state.json` today and will apply to `styles/<name>.css` later.
...
1. **Hot reload of manually-edited per-view override files** — out of scope; blocked on SPEC-30. When SPEC-30 lands, `state/state.json` inherits its hot-reload behavior for free because it lives under `settings/`. Flag in spec so it's not re-debated.
2. **First-run seeding from `layout.json`** — current resolver falls through to `layout.json` declared defaults. Should the new workspace `ai/<machine>/System/styles/state.json` be seeded from the union of declared layouts (per-view layout.json → pick reasonable workspace defaults), or from hardcoded values? Proposal: hardcoded defaults for v1, with a follow-up that walks enabled views' `layout.json` files and picks medians/majorities.
3. **Styles parallel** — this spec establishes the paradigm. The styles version is not landed here. A separate spec (`STYLES_OVERRIDE_SPEC.md`) should follow the same structure: workspace default at `ai/<machine>/System/styles/styles/<name>.css`, per-view override at `ai/<machine>/Views/<view>/settings/styles/<name>.css`, loader merges via CSS cascade (later per-view `<link>` wins naturally). No JSON-merge equivalent needed for CSS — the cascade is the merge.
...
5. **Override file with invalid JSON** — if a user hand-edits `ai/<machine>/Views/<view>/state/state.json` and breaks the syntax, the resolver should log a warning and fall back to workspace-only for that view. Must not crash. Covered by resolver's existing try/catch on read; verify in test.

---

## 14. Acceptance

- Fresh workspace: first launch creates `ai/<machine>/System/styles/state.json` with hardcoded defaults. No per-view override files exist. All views render identically.
...
- Create `ai/<machine>/Views/<wiki-view-folder>/styles/state.json` containing `{ "widths": { "leftChat": 500 } }` by hand. Restart. Wiki-viewer chat renders at 500. Code-viewer still at the workspace value. Drag wiki-viewer's chat to 550 → wiki-viewer's override file updates to 550. Code-viewer unchanged.
