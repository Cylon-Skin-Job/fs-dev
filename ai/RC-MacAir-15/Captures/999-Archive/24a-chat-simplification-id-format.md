...
1. **`ai/<machine>/Captures/003-TODO/specs/24-chat-simplification.md`** — parent spec. 1 minute. Understand the 6-phase layout and what's deferred.
2. **`ai/<machine>/Wiki/enforcement/code-standards/PAGE.md`** — house rules. Pay attention to "delete, don't deprecate", "no backward-compat shims", and "one job per file".
...
```js
   ...
   * Chat Assistants vs Background Workers taxonomy in ai/<machine>/Agents/ —
...
2. **Wipe existing thread data.** Pre-prod, per parent spec:
   ```bash
   sqlite3 ai/<machine>/System/open-robin.db "DELETE FROM threads;"
   rm -rf ai/<machine>/Views/*/chat/
...
4. **Open the UI in a browser.** Load the file-viewer or any panel with chat.
...
7. **Verify the thread ID format.**
   ```bash
   sqlite3 ai/<machine>/System/open-robin.db "SELECT thread_id, name FROM threads;"
   ```
...
- **Do not** relocate chats to `ai/<machine>/Views/chat/`. Phase 24c.
...
3. thread:create + thread:open + thread:open-daily + thread:open-agent
   ...
   taxonomy in ai/<machine>/Agents/. thread:created / thread:opened
