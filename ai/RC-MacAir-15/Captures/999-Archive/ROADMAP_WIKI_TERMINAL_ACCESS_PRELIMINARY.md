...
- `fusion-studio-client/src/lib/resource-path.ts` maps `wiki-viewer` to `ai/<machine>/Wiki`.
...
- `fs-dev` and `System Source Files` use `ai/<machine>/Wiki/index.json` with `sections`.
...
- Some resources, such as Text-To-Speech rules, are referenced from `System Source Files/ai/<machine>/Wiki/...`; those references need to be updated or intentionally decoupled before deleting old content paths.
...
Objective:
- Make `wiki-viewer` resolve to `ai/<machine>/Wiki` instead of `ai/<machine>/Wiki`.
...
- Update `fusion-studio-client/src/lib/resource-path.ts` so copy/send path actions point at `ai/<machine>/Wiki`.
...
Verification:
- Server resolves `wiki-viewer` panel root to `.../ai/<machine>/Wiki` when that folder exists.
- Other views still resolve to their existing `content/` roots.
- Copy path buttons for wiki resources produce `.../ai/<machine>/Wiki/...`.
...
Expected changes:
- Create `System Source Files/ai/<machine>/Wiki/PAGE.md`.
...
- `fusion-studio-server/lib/resources/resolver.js` currently points Text-To-Speech rules at `System Source Files/ai/<machine>/Wiki/system/Text-To-Speech/Rules`. If those rules move, update this resolver and any packaged resource preparation scripts in the same slice or a dedicated resource slice.
...
Targets:
- `Fusion-Home/ai/<machine>/Views/wiki-viewer`
- `Fusion-Home/workspace-templates/fusion-home/ai/<machine>/Views/wiki-viewer`
- `Fusion-Home/workspace-templates/project-repo/ai/<machine>/Views/wiki-viewer`
- `Fusion-Home/workspace-templates/media-studio/ai/<machine>/Views/wiki-viewer`
- `solobooks/ai/<machine>/Views/wiki-viewer`
- `media-editor/ai/<machine>/Views/wiki-viewer` or its current Media Studio equivalent
...
Expected changes:
- Convert `ai/<machine>/Wiki` into `ai/<machine>/Wiki`.
...
Expected changes:
- Search and update remaining `ai/<machine>/Wiki` references.
...
```text
Implement Slice 9 only: add deterministic terminal/tool access to workspace wiki pages under ai/<machine>/Wiki. Do not read old content/index.json, topics.json, sections/articles/groups metadata, or _Guide.md files. Report exact files touched and verification results.
```
