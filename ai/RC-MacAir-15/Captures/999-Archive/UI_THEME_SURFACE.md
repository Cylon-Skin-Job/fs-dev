...
| Chat, thread list, composer, context meter, messages | `ai/<machine>/System/styles/styles/views.css` (imported from `main.tsx`) |
| Code viewer / file explorer chrome | `ai/<machine>/Views/code-viewer/settings/styles/layout.css` (injected per workspace) |
...
1. **Bundled vs injected CSS**  
   ...
   - `ai/<machine>/Views/code-viewer/settings/styles/layout.css` is loaded via workspace style injection — **reload/reconnect** the app so the client refetches it; editing alone does not use Vite HMR for that path.
