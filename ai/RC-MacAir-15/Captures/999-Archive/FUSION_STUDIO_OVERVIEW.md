...
- **Key events:**
  ...
  - `views:added`, `views:removed`, `views:changed` — view lifecycle (deprecated; use `file:changed` filtered for `ai/<machine>/Views/`)
...
### Current State
- **Custom protocol:** `fusion-studio://{viewId}/app/index.html` serves files from `ai/<machine>/Views/{viewId}/`
...
File-based ticket tracking in `ai/<machine>/Issues/`.
...
- **Workspace styles:** `ai/<machine>/System/styles/` — `themes.json`, `themes.css`, `variables.css`, `components.css`, `views.css`, etc.
...
| B | 2.1 | ✅ | Filesystem Restructure (`ai/<machine>/System` → `ai/<machine>/System`) |
...
| Chunk Specs | `ai/<machine>/Views/doc-viewer/specs/chunks/` |
| Ticket Markdown | `ai/<machine>/Issues/{inbox,closed,archived}/` |
| Ticket JSON Index | `ai/<machine>/Issues/content/tickets.json` |
