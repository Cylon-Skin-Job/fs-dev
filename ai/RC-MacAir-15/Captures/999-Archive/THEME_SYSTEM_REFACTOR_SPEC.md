...
**Replaces:** `ai/<machine>/System/styles/THEME_SYSTEM_AUDIT.md` (this spec supersedes it)  
...
A single-source-of-truth theme and settings system that lives entirely in the filesystem (`ai/<machine>/Views/**/settings/`). No database involvement for themes, layout, or CSS.
...
- **Per-view overrides:** Optional `ai/<machine>/Views/<view>/styles/themes.css` scopes to that view only
...
ai/<machine>/Views/<view>/settings/
...
```
User edits ai/<machine>/System/styles/themes.css
...
User edits ai/<machine>/System/styles/themes.json
...
Client loads view = "wiki-viewer"
  ...
  → Server returns:
      { globalCss: "...themes.css contents...",
        viewCss: "...wiki-viewer/styles/themes.css or null...",
        layout: { ...wiki-viewer/styles/layout.json or {}... } }
...
- Reads/writes `ai/<machine>/System/styles/themes.json` and `ai/<machine>/System/styles/themes.css`.
...
Reads/writes `ai/<machine>/Views/<view>/styles/layout.json`. Returns `{}` if file missing.
...
**Acceptance criteria:**
- [ ] Creating `ai/<machine>/Views/<wiki-view-folder>/styles/themes.css` with `--text-primary: red` makes wiki view text red without affecting other views
- [ ] Creating `ai/<machine>/Data/File-Viewer/styles/layout.json` with `{ chatWidth: 400 }` is returned by the API
- [ ] Editing `ai/<machine>/System/styles/themes.json` by hand triggers CSS regeneration via watcher
...
### Paths
- Global settings: `<projectRoot>/ai/<machine>/System/styles/`
- Per-view settings: `<projectRoot>/ai/<machine>/Views/<view>/settings/`
...
| Per-view CSS overrides | 3 | Create `wiki-viewer/styles/themes.css` with `--text-primary: red`. Only wiki view turns red. |
| Per-view layout loads | 3 | Create `file-viewer/styles/layout.json` with `{ sidebarOpen: false }`. API returns it. |
