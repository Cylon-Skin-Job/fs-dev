...
2. Create two databases: `ai/<machine>/System/robin.db` (system) and `ai/<machine>/System/project.db` (project data)
...
- **DB location:** `ai/<machine>/System/` — visible to user (Option B), managed by Robin, in .gitignore
...
`ai/<machine>/System/robin.db` — Option B. Visible to user, managed by Robin. In `.gitignore`.
...
- [ ] Add `ai/<machine>/System/robin.db` to `.gitignore`
- [ ] Add `db.destroy()` to server shutdown handler
- [ ] Verify: server starts, robin.db created in `ai/<machine>/System/`, tables exist
...
### Current Path
```
ai/<machine>/Views/{workspace}/threads/{threadId}/CHAT.md
```

### New Path
```
ai/<machine>/Views/{workspace}/chat/threads/{username}/thread-name.md
```
...
- [ ] `ai/<machine>/System/robin.db` created on server start (4 tables)
