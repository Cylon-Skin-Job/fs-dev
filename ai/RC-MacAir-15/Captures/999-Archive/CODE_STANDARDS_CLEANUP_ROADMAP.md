...
**Source rule:** `ai/<machine>/Wiki/enforcement/code-standards/PAGE.md`
...
- Runtime restarts can dirty workspace/theme state files such as `ai/<machine>/System/state/state.json`, `ai/<machine>/System/styles/themes.css`, `ai/<machine>/System/styles/themes.json`, and `fusion-studio-server/data/workspace-cache.json`. Do not accidentally commit those unless the task is explicitly about persisted workspace/theme state.
...
- For each token, grep both project folders (`~/projects/Fusion-Home/ai/<machine>/System/styles/` and `~/projects/fs-dev/ai/<machine>/System/styles/`) and confirm consistent definitions.
...
3. **Where do tokens get overridden?** Grep `ai/<machine>/System/styles/themes.css` and `ai/<machine>/System/styles/views.css` in **both** Fusion Home and fs-dev project folders. Flag any token whose value differs across the two projects — those are tokens the cleanup must touch carefully.
...
- Possibly `ai/<machine>/System/styles/themes.css` in Fusion Home and fs-dev (to add tokens if missing)
...
**Project CSS interaction:** `App.css` rules can be overridden by `ai/<machine>/System/styles/views.css`. Moving rules to component CSS without updating the project-layer overrides means project overrides stop working. The research step has to flag any class currently overridden in either project folder.
...
For every class moved, confirm `ai/<machine>/System/styles/views.css` (Fusion Home and fs-dev) doesn't re-target it under the old global selector path. Adjust overrides to target the new component selector.
...
Update `ai/<machine>/Wiki/enforcement/code-standards/PAGE.md` and `LOG.md`. Nothing else changes.
...
|---|---|---|
| CSS token swap defeated by project-layer override | Phase 1, all chunks | Mandatory grep of both project `ai/<machine>/System/styles/` folders before token introduction |
