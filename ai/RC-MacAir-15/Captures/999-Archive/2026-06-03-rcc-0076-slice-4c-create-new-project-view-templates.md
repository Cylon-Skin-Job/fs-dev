...
- Selected views are scaffolded into `ai/<machine>/Views/<view-id>` using each manifest entry's `templatePath`.
- The server creates `ai/<machine>/Views/index.json` for compatibility with current code.
...
For default templates, include enough `index.json` metadata for the scaffold service to generate `ai/<machine>/Views/index.json`.
...
- Create `/ai`, `/ai/<machine>/Views`, and needed system folders.
- Copy selected template folders into `ai/<machine>/Views/<view-id>`.
- Generate `ai/<machine>/Views/index.json` from selected manifest entries with id, label, icon, rank.
...
- Destination must resolve under the new project's `ai/<machine>/Views`.
...
- Do not migrate live view registry to `ai/<machine>/System/workspace/views.json`; that belongs to Slice 2b after guard/audit.
...
7. Confirm project folder is created with `/ai` and selected `ai/<machine>/Views/<view-id>` folders.
...
The app has a minimal `Create New` flow backed by a manifest/template catalog in `System Source Files`. New projects are scaffolded with `/ai`, selected view folders, and compatibility `ai/<machine>/Views/index.json`, then registered and added to the ribbon.
