...
- Thread file relocation to `ai/<machine>/Views/chat/threads/` (Phase 24c)
...
1. **`ai/<machine>/Wiki/enforcement/code-standards/PAGE.md`** — house rules. Re-read "delete don't deprecate" and "no backward-compat shims" — both bite hard in this spec.
2. **`ai/<machine>/Captures/003-TODO/specs/24-chat-simplification.md`** — parent spec. Understand which phases are deferred.
3. **`ai/<machine>/Captures/003-TODO/specs/24a-chat-simplification-id-format.md`** — the precursor that introduced timestamp thread IDs. You build on top of its ID format.
4. **`ai/<machine>/Captures/003-TODO/specs/25-frontmatter-separation-of-concerns.md`** — the `lib/frontmatter/` module you'll consume.
...
```bash
grep -rn "^\s*\w*:\s*null\s*$" ai/<machine>/Views/**/settings/TRIGGERS.md ai/<machine>/Issues/tickets/ ai/components 2>/dev/null
```
...
class ChatFile {
   ...
   * @param {string} [opts.viewsDir] - New: ai/<machine>/Views/{workspace}/chat/threads/{username}
...
class ChatFile {
   ...
   *   (e.g. ai/<machine>/Data/Chatlogs/threads/rccurtrightjr.)
...
2. **Wipe pre-prod thread data** (per parent spec — existing threads are disposable).
   ...
   find /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/*/chat/threads -type d -empty -delete 2>/dev/null
   ```
...
5. **Verify the file was created correctly.**
   ```bash
   # Find the file
   find /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/*/chat/threads -name '*.md' -type f
...
10. **Verify SQLite agrees.**
    ```bash
    sqlite3 /Users/rccurtrightjr./projects/open-robin/ai/<machine>/System/robin.db "SELECT thread_id, name FROM threads;"
...
- **Do not** relocate thread files to `ai/<machine>/Views/chat/threads/...`. Phase 24c. Keep per-panel layout for this spec.
