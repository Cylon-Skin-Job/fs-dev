...
| `Captures/` | Workspace document work product, including captures, draft specs, todos, playground notes, assets, screenshots, and document-like material displayed by document-oriented views. |
...
- Docs data belongs in `Captures/`, not `Views/001-doc-viewer/`.
...
Per-view `state/state.json` replaces old global state assumptions such as `ai/<machine>/System/state/state.json` and old per-view override files under `ai/<machine>/Views/<view>/state/state.json`.
...
`Captures/` is the data source for `doc-viewer`.

Folders directly inside `Captures/` become tiled rows in `doc-viewer`.

Root-level documentation files such as `Captures/README.md` are metadata/help text and must be ignored by the tiled row renderer.
...
Files inside each `Captures/<row>/` folder become the tiles for that row. File display labels follow the same convention: strip numeric order prefixes, replace underscores with spaces, and preserve capitalization.

Root-level files directly under `Captures/` do not become tiles. Only files inside row folders are rendered as document tiles.
...
- Removing a view must not delete bound data folders such as `Captures/`, `Wiki/`, `Issues/`, `Agents/`, `Office/`, or selected media/library folders.
...
- Workspace templates may include documentation in `Captures/`, `Wiki/`, or another appropriate top-level data folder.
...
The old `ai/<machine>/System/workspace/views.json` and `ai/<machine>/System/state/workspace.json` roles are replaced by v2 `Views/` folder order, per-view `manifest.md`, per-view `styles/icon.md`, and `System/state/` records for hidden/restorable view state.
...
- Planned modes may include:
  - `Read`
  - `Capture` — read plus write access to Captures/doc-viewer capture workflows.
...
As each legacy view under `ai/<machine>/Views/<viewer>/` is converted to the v2 `Views/00X-<viewer>/` shape, remove or migrate anything that violates the view-shell-only rule.
