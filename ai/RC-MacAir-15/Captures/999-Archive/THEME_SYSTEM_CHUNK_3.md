...
Currently:
- `themes.css` lives at `ai/<machine>/System/styles/themes.css` and is global
- `themes.json` lives at `ai/<machine>/System/styles/themes.json`
- The client loads CSS via WebSocket `file_content_request` for `styles/themes.css`
- Some views already have `layout.json` (e.g., `file-viewer/styles/layout.json`) but there is no unified serving mechanism

We want:
- Power users can drop `ai/<machine>/Views/<view>/styles/themes.css` to override tokens for that view only
- Each view can have `ai/<machine>/Views/<view>/styles/layout.json` for view-specific layout state
...
Add the fetch there when a panel becomes active. If the client already fetches `styles/themes.css` via WS, replace or supplement that path with the REST endpoint.
...
projectWatcher.addFilter({
  ...
  match: ['ai/<machine>/System/styles/themes.json'],
...
- [ ] `GET /api/view-config?panel=wiki-viewer` returns `{ globalCss, viewCss, layout }`
  - `globalCss` contains the contents of `ai/<machine>/System/styles/themes.css`
  - `viewCss` contains the contents of `ai/<machine>/Views/<wiki-view-folder>/styles/themes.css` (if file exists) or `""`
  - `layout` contains the parsed JSON of `ai/<machine>/Views/<wiki-view-folder>/styles/layout.json` (or `{}`)
- [ ] Creating `ai/<machine>/Views/<wiki-view-folder>/styles/themes.css` with `:root { --text-primary: red; }` makes the wiki view's text red
  - Verify by loading the wiki view and checking computed styles
- [ ] Creating `ai/<machine>/Data/File-Viewer/styles/layout.json` with `{ sidebarOpen: false }` is returned by the API
- [ ] Editing `ai/<machine>/System/styles/themes.json` by hand triggers `themes.css` regeneration within 1 second
...
```bash
mkdir -p ai/<machine>/Views/<wiki-view-folder>/settings
echo ':root { --text-primary: red; }' > ai/<machine>/Views/<wiki-view-folder>/styles/themes.css
curl "http://localhost:3001/api/view-config?panel=wiki-viewer"
```

**Test layout:**
```bash
mkdir -p ai/<machine>/Data/File-Viewer/settings
echo '{ "chatWidth": 400 }' > ai/<machine>/Data/File-Viewer/styles/layout.json
...
vim ai/<machine>/System/styles/themes.json
# Within 1 second, themes.css should update (check mtime)
stat ai/<machine>/System/styles/themes.css
```
