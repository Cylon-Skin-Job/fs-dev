...
- `ai/<machine>/System/styles/styles/views.css` (global workspace CSS) — minor touch
...
- Each view has its own UI state file at `ai/<machine>/Views/<view>/state/<username>.json`
...
- Widths persist per view per user at `ai/<machine>/Views/<view>/state/<username>.json`
...
**D1 — File location.** `ai/<machine>/Views/<view>/state/<username>.json`. New `state/` subdirectory per view. Matches "each view is self-contained" pattern.
...
1. Per-user file (`ai/<machine>/Views/<view>/state/<username>.json`)
...
2. **`ai/<machine>/Captures/003-TODO/specs/26a-dual-chat-data-model.md`** — understand the data model that the scope infrastructure rests on.
3. **`ai/<machine>/Captures/003-TODO/specs/26b-dual-chat-routing-layer.md`** — understand the server routing.
4. **`ai/<machine>/Captures/003-TODO/specs/26c-dual-chat-client-layout.md`** — understand what 26c delivered (and what you're partially undoing).
...
find /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/*/chat/threads -type f -name '*.md' -delete 2>/dev/null
rm -rf /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/*/state/ 2>/dev/null
```
 ...
 * ai/<machine>/Views/<view>/state/<username>.json. Reads fall back through a
...
cd /Users/rccurtrightjr./projects/open-robin
pkill -9 -f "node.*server.js" 2>/dev/null; sleep 1
sqlite3 ai/<machine>/System/robin.db "PRAGMA foreign_keys=ON; DELETE FROM threads;"
find ai/<machine>/Views/chat/threads -type f -name '*.md' -delete 2>/dev/null
find ai/<machine>/Views/*/chat/threads -type f -name '*.md' -delete 2>/dev/null
rm -rf ai/<machine>/Views/*/state/ 2>/dev/null
...
16. ```bash
    cat /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Data/File-Viewer/state/rccurtrightjr..json
    ```
...
- **Do not** write per-view state to any location other than `ai/<machine>/Views/<view>/state/<username>.json`.
...
Files at ai/<machine>/Views/<view>/state/<username>.json hold:
  {
