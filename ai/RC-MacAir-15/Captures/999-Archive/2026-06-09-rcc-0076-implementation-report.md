...
- Preserved legacy `ai/<machine>/Views/index.json` as compatibility output only.
...
- New projects write primary live registry:
  - `ai/<machine>/System/workspace/views.json`
- Legacy `ai/<machine>/Views/index.json` remains compatibility output.
...
- View-management mutations update only `ai/<machine>/System/workspace/views.json`.
...
ai/<machine>/Views/<view>/state/state.json
```
...
- Live workspace view registry is `ai/<machine>/System/workspace/views.json`.
- Legacy `ai/<machine>/Views/index.json` is compatibility-only output.
