...
- **Storage paths branch by scope.**
  - Project threads → `ai/<machine>/Views/chat/threads/<user>/<id>.md` (the 24c unified location)
  - View threads → `ai/<machine>/Views/<view>/chat/threads/<user>/<id>.md` (per-view path, restored)
...
3. **The unified `ai/<machine>/Views/chat/threads/<user>/` location stays** — it becomes the home for project-scoped threads only. View-scoped threads stop writing there.
...
1. **`ai/<machine>/Wiki/enforcement/code-standards/PAGE.md`** — house rules.
2. **`ai/<machine>/Captures/003-TODO/specs/26a-dual-chat-data-model.md`** — what 26a delivered. Understand that the SQLite schema is already dual-scope-ready; 26b is plumbing only.
3. **`ai/<machine>/Captures/003-TODO/specs/26b-dual-chat-routing-layer.md`** — this spec.
...
```bash
echo "=== threads ==="
sqlite3 /Users/rccurtrightjr./projects/open-robin/ai/<machine>/System/robin.db \
  "SELECT thread_id, project_id, scope, view_id, name FROM threads;"
echo ""
echo "=== filesystem (unified location) ==="
find /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/chat -type f -name '*.md' 2>/dev/null
echo ""
 ...
 *   - project scope → ai/<machine>/Views/chat/threads/<user>/  (the 24c unified location)
 *   - view scope    → ai/<machine>/Views/<view>/chat/threads/<user>/  (per-view, restored from pre-24c)
 *
 * @returns {string}
 */
_getViewsDir() {
...
# Wipe SQLite threads
sqlite3 /Users/rccurtrightjr./projects/open-robin/ai/<machine>/System/robin.db "PRAGMA foreign_keys=ON; DELETE FROM threads;"
...
# Verify clean
sqlite3 /Users/rccurtrightjr./projects/open-robin/ai/<machine>/System/robin.db "SELECT COUNT(*) FROM threads; SELECT COUNT(*) FROM exchanges WHERE thread_id NOT IN (SELECT thread_id FROM threads);"
# Expected: 0 / 0
```
...
try { new ThreadManager({ scope: 'view', viewId: 'file-viewer', projectRoot: '/tmp/fake' }); console.log('PASS: view scope'); } catch (e) { console.log('FAIL view scope:', e.message); }
...
const viewMgr = new ThreadManager({ scope: 'view', viewId: 'file-viewer', projectRoot: '/tmp/fakeproject' });
const projectPath = projectMgr._getViewsDir();
const viewPath = viewMgr._getViewsDir();
console.log('project path:', projectPath);
console.log('view path:', viewPath);
const projectOK = projectPath.includes('/ai/<machine>/Views/chat/threads/');
const viewOK = viewPath.includes('/ai/<machine>/Data/Chatlogs/threads/');
...
ws.on('open', async () => {
  ...
  // 1. set_panel to file-viewer (sets up both ThreadManagers per the new setPanel)
  send({ type: 'set_panel', panel: 'file-viewer' });
  await wait(500);
...
- Row 2: `scope='view'`, `view_id='file-viewer'`, `project_id='open-robin'`

```bash
# Project thread file in the unified location
find /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Views/chat -type f -name '*.md'

# View thread file in the per-view location
find /Users/rccurtrightjr./projects/open-robin/ai/<machine>/Data/File-Viewer/chat -type f -name '*.md' 2>/dev/null
```

Expected:
- One file under `ai/<machine>/Views/chat/threads/<user>/` (the project thread)
- One file under `ai/<machine>/Data/Chatlogs/threads/<user>/` (the view thread)
...
- **Do not** delete the unified `ai/<machine>/Views/chat/threads/<user>/` directory after switching project threads to write there. It's the home for project-scoped threads.
...
data model. Each WS connection now holds TWO ThreadManagers:
  - projectThreadManager (scope='project') — persistent across panel
    switches, writes to ai/<machine>/Views/chat/threads/<user>/
  - viewThreadManager (scope='view') — swaps on panel change, writes
    to ai/<machine>/Views/<view>/chat/threads/<user>/ (per-view path restored)
...
Live-validated via /tmp/26b-validate.mjs (raw WS test script):
  - Creates one view thread → lands in
    ai/<machine>/Data/Chatlogs/threads/<user>/<id>.md
    with scope='view', view_id='file-viewer' in SQLite
