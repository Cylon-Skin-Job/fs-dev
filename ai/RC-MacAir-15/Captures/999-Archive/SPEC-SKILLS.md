...
Skills live in `ai/<machine>/System/skills/`. They ship with the app. Users can add their own scripts here. Per-workspace skills (in `tools.json`) supplement the system skills for domain-specific operations.

### Two Skill Sources

| Source | Location | Scope | Purpose |
|--------|----------|-------|---------|
| **System skills** | `ai/<machine>/System/skills/manifest.json` | All agents, all projects | Universal capabilities (search, lookup, trace) |
| **Workspace skills** | `ai/<machine>/Views/{workspace}/tools.json` | Agents in that workspace | Domain-specific (create-invoice, import-contacts) |
...
### System Manifest (`ai/<machine>/System/skills/manifest.json`)
...
### Workspace Skills (`ai/<machine>/Views/{workspace}/tools.json`)
...
```javascript
// ai/<machine>/System/skills/lookup-events.js
...
const systemManifest = loadManifest('ai/<machine>/System/skills/manifest.json');
...
~/.qwen/skills/lookup-events → symlink to ai/<machine>/System/skills/lookup-events.js
```
...
- [ ] Create `ai/<machine>/System/skills/` directory with manifest.json
