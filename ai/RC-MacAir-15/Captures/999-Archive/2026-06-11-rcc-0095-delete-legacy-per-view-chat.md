...
The current `ai/<machine>/Views/<view>/chat/` folders are misleading legacy artifacts. For example, `ai/<machine>/Views/doc-viewer/chat/settings/` is empty, but server code still treats `ai/<machine>/Views/doc-viewer/chat/` as a capability marker.
...
This makes `ai/<machine>/Views/<view>/chat/` control whether a view reports `hasChat: true`.
...
```text
ai/<machine>/Views/<view>/chat/threads/<user>/
```
...
```text
ai/<machine>/Views/chat/threads/<user>/
```
...
- `ai/<machine>/Views/*/chat/` when only used as empty marker/settings shell.
- `System Source Files/ai-template/ai/<machine>/Views/*/chat/` if present.
...
```text
ai/<machine>/Views/doc-viewer/chat/settings/
```
...
- Confirm no code path creates `ai/<machine>/Views/<view>/chat/threads/`.
- Confirm empty `ai/<machine>/Views/<view>/chat/` folders are no longer required for `hasChat`.
