...
- `.rv-tool-fade-in` — add `margin-bottom` (existing rule in `ai/<machine>/System/styles/views.css` or `App.css`)
...
**CSS file targets:** locate via `grep -r "rv-tool-fade-in\|rv-emoji\|rv-fusion-color-picker-label\|rv-fusion-detail-body\|rv-file-tree-item\|rv-voice-recorder" fusion-studio-client/src ai/<machine>/System`
...
| Rule(s) | File |
|---|---|
| `.rv-tool-*` (5 new rules) | `ai/<machine>/System/styles/views.css` — tool-call section |
| `.rv-emoji-*` (5 new rules) | New `fusion-studio-client/src/emojis/EmojiTrigger.css` (imported by `EmojiTrigger.tsx`) |
| `.rv-file-tree-empty-label` | `ai/<machine>/Data/File-Viewer/styles/layout.css`; mirror to `Fusion-Home` layout |
| `.rv-voice-recorder__kitt-bar` update | Existing mic CSS file (grep for the class) |
| `.rv-hover-icon-modal-positioner` | `ai/<machine>/System/styles/views.css` — modal section |
| `.rv-wf-step-spacer`, `.rv-wf-*` | `ai/<machine>/System/styles/views.css` — agents section |
| `.rv-thread-jump-row`, `.rv-thread-rename-input` | `ai/<machine>/System/styles/views.css` — threads section |
| `.rv-content-placeholder-*` | `fusion-studio-client/src/components/App.css` — ContentArea section |
| `.rv-ticket-*` or `.rv-dim-label` | `ai/<machine>/System/styles/views.css` — tickets section |
...
**Mirror rule:** any new class added to `ai/<machine>/System/styles/views.css` in fs-dev must be added to the matching section in `~/projects/Fusion-Home/ai/<machine>/System/styles/views.css`.
