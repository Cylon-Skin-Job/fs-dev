...
This roadmap captures the current findings and proposed implementation path for removing `ai/<machine>/Views/chat` from app view navigation while preserving chat markdown as a system-managed audit/export layer.

The goal is to make chat mandatory app behavior, not a folder-detected optional view, and to move human-readable thread markdown into `ai/<machine>/System/` with a naming/frontmatter convention aligned with the wiki.

## Product Decision

Chat is mandatory.

The server should not ask the filesystem whether chat exists. It should not use `ai/<machine>/Views/<view>/chat/` or `ai/<machine>/Views/chat/` folder presence as the source of truth for whether chat renders or functions.
...
### `ai/<machine>/Views/chat` currently appears because it is registered as a view

`ai/<machine>/System/workspace/views.json` contains:
...
Because `ai/<machine>/Views/chat` exists, the registry makes it visible in app view navigation.

### `ai/<machine>/Views/chat` currently stores project chat markdown
...
```text
ai/<machine>/Views/<view>/chat/threads/<user>/<threadId>.md
```
...
Chat audit markdown should move out of `ai/<machine>/Views/` and into `ai/<machine>/System/`.

Recommended project-scoped path:

```text
ai/<machine>/System/chat/threads/<user>/<thread-id>/CHAT.md
```

Recommended view-scoped path:

```text
ai/<machine>/System/chat/views/<view-id>/threads/<user>/<thread-id>/CHAT.md
```

Rationale:

- `ai/<machine>/Views/` remains only for navigable app views.
- `ai/<machine>/System/chat/` clearly communicates system-managed audit/export data.
...
- Which code paths create `ai/<machine>/Views/chat`?
- Which code paths create `ai/<machine>/Views/chat/threads`?
...
- Does any code treat `ai/<machine>/Views/chat` as a view folder beyond `views.json` registration?
...
- Move existing `ai/<machine>/Views/chat/threads` files into `ai/<machine>/System/chat`, leave them in place, or archive them?
...
- Should migration happen before or after new writes switch to `ai/<machine>/System/chat`?
...
- Project-scoped audit files should live at `ai/<machine>/System/chat/threads/<user>/<thread-id>/CHAT.md`.
- View-scoped audit files should live at `ai/<machine>/System/chat/views/<view-id>/threads/<user>/<thread-id>/CHAT.md`.
...
- Existing `ai/<machine>/Views/chat/threads` data should not be deleted without explicit approval.
...
- Do not register `ai/<machine>/Views/chat` as an app view.
...
Objective: stop `ai/<machine>/Views/chat` from appearing as a navigable app view.

Steps:

1. Remove or disable the `chat` entry in `ai/<machine>/System/workspace/views.json`.
2. Ensure `listRegistryViews()` continues to list real app views only.
3. Do not delete existing `ai/<machine>/Views/chat/threads` data in this slice.
...
### Slice 3: Move Markdown Audit Storage To `ai/<machine>/System/chat`
...
2. Write project threads to `ai/<machine>/System/chat/threads/<user>/<thread-id>/CHAT.md`.
3. Write view threads to `ai/<machine>/System/chat/views/<view-id>/threads/<user>/<thread-id>/CHAT.md`.
...
- New project thread creates `ai/<machine>/System/chat/threads/<user>/<thread-id>/CHAT.md`.
- New view thread creates `ai/<machine>/System/chat/views/<view-id>/threads/<user>/<thread-id>/CHAT.md`.
...
- Should existing `ai/<machine>/Views/chat/threads` files be moved, archived, or left untouched until a cleanup slice?
...
- Do not keep `chat` registered as an app view once audit storage moves under `ai/<machine>/System`.
