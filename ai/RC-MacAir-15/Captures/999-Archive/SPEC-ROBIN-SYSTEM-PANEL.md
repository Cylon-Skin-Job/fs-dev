...
```json
{
  ...
  "script": "ai/<machine>/System/skills/lookup-system.js",
  "args": {
...
2. **No per-workspace database.** Project-specific settings (triggers, sessions, prompts, styles, workflows) are files in the repo under `ai/<machine>/Views/*/`. The Settings Viewer reads them live from the filesystem.
