...
**Dev-time quirk:** Open Robin is currently being meta-developed inside Open Robin. `ai/<machine>/System/robin.db` lives inside this repo as a development accident. In the production Electron build, the Open Robin repo is not present on the user's machine — they only see workspaces for their own repos. Treat the current location as transitional.
...
3. The basic folder shape is present: at minimum `ai/<machine>/Views/index.json`. Anything missing the proper structure is silently culled from the registry.
...
- The Electron migration of the system DB out of `ai/<machine>/System/`.
- Replacing the dev-time `ai/<machine>/System/robin.db` with `userData` storage.
