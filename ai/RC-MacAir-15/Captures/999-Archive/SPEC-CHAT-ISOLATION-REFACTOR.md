...
Chat threads from file-viewer bleed into every other panel. The root cause is not one bug — it's a stack of architectural shortcuts that all compound.
...
`setCurrentPanel()` only updated local Zustand state. It never sent a `set_panel` WebSocket message to the server. The server was permanently stuck on `file-viewer`'s ThreadManager for the entire session.

**Status:** Fixed (2026-04-03). `setCurrentPanel` now sends `{ type: 'set_panel', panel: id }` to the server.

---

### 3. Server Hardcoded `file-viewer` on Connect (Server — server.js)

**File:** `kimi-ide-server/server.js`
**Lines:** ~562-567

On every new WebSocket connection, the server called `ThreadWebSocketHandler.setPanel(ws, 'file-viewer', ...)`. This was needed because wire spawning depends on the ThreadManager existing. But it also meant the thread list was scoped to file-viewer until the client sent `set_panel`.

The original code also immediately sent the thread list on connect (before the client identified itself), which was removed.

**Status:** Partially fixed. `setPanel` still defaults to `file-viewer` (needed for wire dependency), but the thread list is no longer sent on connect — it waits for `set_panel`.
...
To enable chat for a new panel: create `ai/<machine>/Views/{panelId}/chat/threads/index.json` with `{ "sort": "last-active", "order": "desc" }`. The filesystem probe in `panels.ts` will detect it automatically.
...
- [ ] Switch from file-viewer to issues-viewer → thread list is empty (no chat folder)
- [ ] Switch back to file-viewer → thread list shows file-viewer threads only
- [ ] Create thread in file-viewer → does NOT appear in any other panel
...
Filesystem:
  ai/<machine>/Views/{panelId}/chat/threads/index.json  ← Existence = chat enabled
```
