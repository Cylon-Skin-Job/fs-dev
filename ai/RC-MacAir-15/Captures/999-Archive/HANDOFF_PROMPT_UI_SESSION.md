...
**kimi-claude** — Kimi IDE: `kimi-ide-client/` (React + Vite + Zustand), `kimi-ide-server/` (WebSocket bridge). Active UI code is under **`kimi-ide-client/src/`** and **`ai/<machine>/Views/`** (workspace styles). After edits under `kimi-ide-client/src/`, run **`npm run build`** in `kimi-ide-client/`; the server serves **`dist/`**.
...
1. **Two CSS delivery paths**: `views.css` is **bundled** via `main.tsx`. **`ai/<machine>/Views/code-viewer/settings/styles/layout.css`** is **workspace-injected** — changes need **reload/reconnect**, not only mental model; **`npm run build`** does not bundle that file into the same pipeline as `views.css`.
