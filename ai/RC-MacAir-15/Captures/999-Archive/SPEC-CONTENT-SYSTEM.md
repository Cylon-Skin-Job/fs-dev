...
Every panel in `ai/<machine>/Views/` follows this layout:
...
**Used by:** agents-viewer, doc-viewer.
...
**Pointer files:** For cases where content lives outside `ai/<machine>/Views/` (e.g., file-viewer pointing to the project repo root), use a `CONTENT.md` with a path declaration instead of symlinking into yourself:
...
Hierarchical document reader for books, scientific studies, and reference material. Structurally similar to `file-explorer` but designed for reading, not code. Data lives in the project repo (like file-viewer), not in its own `content/` folder.
...
- **Pointer files** for self-referential cases. A `CONTENT.md` with frontmatter declaring `root: "${PROJECT_ROOT}"` tells the system to serve content from that path. Used by file-viewer to point to the repo without creating a recursive symlink.
- **Cross-project composability.** Symlink another project's capture folder into your doc-viewer to see both side by side (as separate rows in `tiled-rows` or separate tabs if `tabbed`).
...
|---|---|---|---|
| doc-viewer | `tiled-rows` | Yes | Folders = row headers, items = tiles with code/image preview |
| file-viewer | `file-explorer` | Yes | CONTENT.md pointer to project root |
...
**Removed:**
- `ai/wiki-data/` → moved into `ai/<machine>/Wiki/`
- `ai/wiki/` → deleted (stale duplicate)
- `ai/components/` → moved into `ai/<machine>/Views/settings-viewer/`
