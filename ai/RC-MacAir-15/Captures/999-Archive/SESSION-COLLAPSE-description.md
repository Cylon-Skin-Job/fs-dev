...
### 8. `ai/<machine>/Wiki/system-tools/secrets-manager/PAGE.md` (modify)
...
9. **Wiki article matches new schema.** `ai/<machine>/Wiki/system-tools/secrets-manager/PAGE.md` has zero references to `use_when`. The example `get-secret-list` JSON output in the article matches the new shape exactly.
