...
|---|---|---|---|
| **Workspace-universal** | `ai/<machine>/Views/chat/threads/<user>/` | Always — main chat panel, popup bubble | The primary conversation surface |
| **View-bound** | `ai/<machine>/Views/<viewer>/chat/threads/<user>/` | Only when that view is open | Wiki chat, agents chat |
...
- **Thread storage paths** — already correct per SPEC-26b (`ai/<machine>/Views/chat/threads/` for project scope, `ai/<machine>/Views/<view>/chat/threads/` for view scope).
