...
Identical to `STATE_OVERRIDE_SPEC` §3. A new top-level `tints` key extends `ViewState`. Workspace default lives at `ai/<machine>/System/styles/state.json`; per-view override at `ai/<machine>/Views/<view>/state/state.json`. Deep merge per key.
...
```jsonc
// ai/<machine>/System/styles/state.json — full tints block (v1 baseline)
{
  "widths":   { ... },
  "collapsed":{ ... },
  "popup":    { ... },
...
```jsonc
// ai/<machine>/Views/<wiki-view-folder>/styles/state.json — wiki re-enables card tinting only
{
...
|------|----------|---------|-------------|
| `ai/<machine>/System/styles/styles/views.css:13` | `.sidebar` | `background: var(--sidebar-surface-bg, #161616)` | `background: color-mix(in srgb, var(--sidebar-surface-bg) 88%, var(--ws-primary) 12%)` |
...
| File | Selector | Current | When `true` |
|------|----------|---------|-------------|
| `ai/<machine>/Views/code-viewer/settings/styles/layout.css:53` | `.file-tree-sidebar` | `background: var(--sidebar-surface-bg, #161616)` | `color-mix(in srgb, var(--sidebar-surface-bg) 88%, var(--ws-primary) 12%)` |
...
| File | Selector | Current | When `true` |
|------|----------|---------|-------------|
| `ai/<machine>/System/styles/styles/views.css:15` | `.sidebar` `border-right` | `1px solid var(--neutral-chrome-border)` | `1px solid var(--theme-border)` |
...
| File | Selector | Current | When `true` |
|------|----------|---------|-------------|
| `ai/<machine>/System/styles/styles/views.css:59` | `.chat-area` `border-right` | `1px solid var(--neutral-chrome-border)` | `1px solid var(--theme-border)` |
...
**`ai/<machine>/System/styles/styles/views.css`**
...
**`ai/<machine>/Views/code-viewer/settings/styles/layout.css`**
...
| `ai/<machine>/System/styles/styles/views.css`                          | Add `.rv-panel[data-tint-left]`, `[data-tint-border-threads]`, `[data-tint-border-chat]` rules. |
| `ai/<machine>/Views/code-viewer/settings/styles/layout.css`             | Add `.rv-panel[data-tint-right]` rule for `.file-tree-sidebar`. |
...
| `ai/<machine>/System/styles/state.json`                                | Add `tints` block with all-false defaults on next server boot (server auto-writes the key during resolver normalization — see §12.3). |
...
- Edit `ai/<machine>/System/styles/state.json` to set `tints.cards: true`. Restart. Both wiki and issues regain their accent tinting exactly as it looked pre-spec.
- Edit `ai/<machine>/System/styles/state.json` to set `tints.leftPanel: true`. Restart. Threads sidebar background picks up a subtle accent blend. Chat column and right column unchanged.
- Edit `ai/<machine>/System/styles/state.json` to set `tints.borders.threads: true`. Restart. Threads column's right border switches from neutral to `--theme-border`. Chat column border unchanged.
- Drop `"tints": { "cards": true }` into `ai/<machine>/Views/<wiki-view-folder>/styles/state.json`. Workspace-global `tints.cards` still `false`. Wiki shows accents; issues stays neutral.
