...
**How detected:** Filesystem CSS at `ai/<machine>/System/styles/themes.css` matches `system_theme.theme_css` from the database.
...
**What the toggle did:** Copied `system_theme.theme_css` → `ai/<machine>/System/styles/themes.css`
...
**What Apply does:**
1. Reads filesystem CSS from `ai/<machine>/System/styles/themes.css`
...
│  ## What happened                           │
│                                             │
│  The CSS at `ai/<machine>/System/styles/themes.css`  │
...
// 2. Write it to ai/<machine>/System/styles/themes.css
...
```javascript
// 1. Read ai/<machine>/System/styles/themes.css from filesystem
...
- **Multiple workspaces sharing the same filesystem path:** Not possible — each workspace has its own `ai/<machine>/Views/` path. But if workspace switching isn't built yet, only the active workspace's path exists.
...
- **Filesystem (AI can read):** `ai/<machine>/System/styles/themes.css` is readable by AI but is a propagated copy, not source of truth. AI editing this file triggers State 3 (diverged) on next panel render.
