...
```js
{ type: 'file:changed', path: 'ai/<machine>/Issues/open/RCC-0077.md', change: 'add' }
{ type: 'file:changed', path: 'ai/<machine>/Issues/open/RCC-0071.md', change: 'modify' }
{ type: 'file:changed', path: 'ai/<machine>/Issues/done/RCC-0065.md', change: 'add' }
```
...
**Bot detection:** read from `ai/<machine>/Issues/settings/registry.json` (not hardcoded). Fall back to hardcoded set if file missing.
...
type: ticket
template: ai/<machine>/Issues/templates/deploy-checklist.md
assignee: rccurtrightjr
priority: interrupt
---
```
