...
Defined in `ai/<machine>/System/styles/themes.css`. The picker writes this file. §4 enumerates the exhaustive list.
...
2. **Workspace theme** — `ai/<machine>/System/styles/themes.css` overrides `--theme-primary` + accent family + chrome fills for the whole workspace. Loaded globally by `useSharedWorkspaceStyles`.
3. **Per-view theme** — `ai/<machine>/Views/<view>/styles/themes.css` if present, scoped to `[data-panel="<view>"]` by `useViewLayoutStyles`. Overrides only the tokens the file explicitly sets; unspecified tokens fall through to workspace.
...
A **theme slug** is one entry in `ai/<machine>/System/styles/themes.json`. The file is the canonical source of truth — no SQLite, no separate database. A slug renders deterministically to one `themes.css` file.
...
1. **`ai/<machine>/System/styles/themes.json`** — canonical source of truth. Stores all slugs. The active slug is the one with `"active": true`.
2. **`ai/<machine>/System/styles/themes.css`** — derived from the active slug. This is what the CSS cascade reads.
...
**Invariant:** `themes.css` is always derived from the active slug in `themes.json`. Hand-edits to `themes.css` survive until the next picker write. Power users wanting permanent CSS divergence use per-view overrides (`ai/<machine>/Views/<view>/styles/themes.css`) — the picker never touches those.

**Per-workspace isolation:** `themes.json` lives inside the workspace (`ai/<machine>/System/styles/`). Different workspaces have different theme catalogs automatically. A workspace can carry its own builtin themes and user themes without any cross-contamination.

---

## 8. Per-view override semantics

Per-view `ai/<machine>/Views/<view>/styles/themes.css` works exactly like today (loader already wired):
...
| `ai/<machine>/System/styles/themes.css` | exists | comment header points at this contract |
| `ai/<machine>/System/styles/themes.json` | exists | 59 builtin slugs extracted from getdesign CLI; schema matches §6 |
...
- Studio mockup at `ai/<machine>/Captures/004-Playground/theme-studio.html` — live preview of the full picker surface
...
- The active `ai/<machine>/System/styles/themes.css` in the repo contains only tokens from §4 (accent family + chrome fills). No tokens from §3 (neutrals, text, status, scrollbar, syntax, layout) appear in it.
...
- `grep -n "var(--theme-primary\|--ws-primary)" open-robin-client/src ai/<machine>/Views` returns the expected consumer surface; nothing uses `--color-primary` or similar legacy names (tracking §12.1 follow-up).
