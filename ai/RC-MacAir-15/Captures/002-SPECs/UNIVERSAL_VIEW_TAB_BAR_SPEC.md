...
| Per-view routing | `state:set` a `docViewerTabs` patch → key appears in `System/Views/<id>/state/state.json`, NOT in shared `System/state/state.json` |
...
controller patch → setViewState (RAM) + _persistViewPatch
                 ...
                 → ai/<machine>/System/Views/001-capture-viewer/state/state.json   ← per-view capsule
...
```jsonc
// excerpt of System/Views/<id>/state/state.json
{
  "docViewerTabs": [
    {
