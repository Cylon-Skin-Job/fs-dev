...
The resolver must use `workspace/ai-paths.js` helpers rather than hardcoding legacy `ai/<machine>/System` paths.
...
> **Provenance boundary (2026-07-15):** The runner-facing canonical tool result in this SPEC is not the settled provenance captured-output schema. Output serialization, hashes, byte counts, truncation, external storage, and redaction remain governed by the owner-selected, registered resolution of [provenance schema finding 3](../008-Provenance-Temp/provenance-schema-findings.md); this SPEC must not create a competing tool-only provenance vocabulary. This correction follows that findings file and owner direction in chat and leaves the schema choice open.
