...
- `FUSION_HOME.md` is the broad analytical model of the default Fusion Home folder, addable workspace families, plugin principle, project-first left navigation, project creation and Launchpad composition, suite consolidation, and thread-density implications.
- `THREAD_MANAGEMENT.md` is the broad analytical model of content-to-thread actions, narrow conversation creation and registration, ticket-derived identity, view-level guidance, ticket-agent activation, and Skills-defined workflows.
- `HEARTBEATS_AND_MONITORING.md` is the broad analytical model of conditional heartbeat controls, AI-created monitors, assistant- or plugin-supplied variables, opt-in event filtering, proportional wake/action/sleep behavior, and long-running project monitoring.
...
- `PLUGINS_AND_SETTINGS.md` is the broad analytical model of lightweight core settings, per-workspace theming, deeper customization plugins, composable operational plugins, opt-in personal-context Wiki variants, folder-based bundles, permission-bearing Universal Event Bus controls, reusable plugin dependencies and model resources, and registry-backed consent before activation or execution.
- `ROUTINES_AND_AUTOMATIONS.md` is the broad analytical model of explicit routine creation, `triggers.md`-backed semantic graphs, authorized wake conditions, proportional heartbeat execution, visible fronting assistants, scriptable inputs and outputs, plugin dependencies, shared resources, and inspectable permissions.
- `PROJECTS_AND_LAUNCHPAD.md` is the broad analytical model of Project Manager as the renamed Capture surface, project creation and template cloning, universal folder structure, Markdown-first navigation, bulletin surfaces, view-configured guidance, conversation families, Launchpad composition, and hosted tabs and side agents.
