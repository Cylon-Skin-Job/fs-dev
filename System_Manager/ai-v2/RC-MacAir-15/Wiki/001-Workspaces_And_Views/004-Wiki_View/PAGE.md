...
- The right sidebar appears only when a top-level article folder is selected.
- Right-sidebar clicks change the viewed page without changing the right-sidebar context.
- Wiki page frontmatter uses the OpenCode skill-style `---` YAML delimiter pattern.
...
- [Frontmatter Model](001-Architecture/006-Frontmatter_Model/PAGE.md) - `name`, `description`, and `metadata` schema.
