---
name: capture-docs
description: Save loose ideas, exploratory discussion, and lightweight todo items into durable workspace docs for later reference.
---
...
- "Let's create a capture doc."
- "Capture these for later."
- "Let's make a ToDo list."

## Destinations

- Use `ai/docs/001-Captures/` for loose, exploratory, not-yet-actionable ideas.
- Use `ai/docs/004-ToDo/` for small actionable items that are somewhat decided but do not need a spec.
- Reference `ai/docs/001-Captures/README.md` and `ai/docs/004-ToDo/README.md` when present.
...
- If the idea is abstract or still forming, create or update a capture doc.
...
Capture docs should be easy to reference later. Prefer concise markdown with:
...
To-Do docs should be simple and actionable. Prefer:
