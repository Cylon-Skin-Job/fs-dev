...
rule_files:
  - ../Rules/replacement-rules.json
  - ../Rules/list-markers.json
  - ../Rules/correction-flags.json
  - ../Rules/cleanup-thresholds.json
  - ../Rules/filler-rules.json
  - ../Rules/model-pass-rules.json
  - ../Rules/stt-notice-rules.json
  - ../Rules/visual-text-rules.json
---
...
The safe edit surface for cleanup behavior is the `Rules/` folder next to this article.

- [Replacement Rules](../Rules/replacement-rules.json) — user vocabulary and speech-to-text phrase corrections.
- [List Markers](../Rules/list-markers.json) — dictated list start phrases, end phrases, and number marker normalization.
- [Correction Flags](../Rules/correction-flags.json) — phrases that indicate the speaker may be correcting themselves.
- [Cleanup Thresholds](../Rules/cleanup-thresholds.json) — deterministic list-boundary and cleanup thresholds.
- [Filler Rules](../Rules/filler-rules.json) — safe removal rules for filler words such as “um,” “uh,” and contextual “like.”
- [Model Pass Rules](../Rules/model-pass-rules.json) — archived/narrow model-pass definitions; not active for mic cleanup while model cleanup is disabled.
- [STT Notice Rules](../Rules/stt-notice-rules.json) — XML caution notice behavior when dictated text is sent with little review.
- [Visual Text Rules](../Rules/visual-text-rules.json) — UI label and feature-name handling where the system warns instead of rewriting risky text.
