...
function createCanonicalHarnessEventBridge({
  ...
  function claimOwner(lifecycle, ws, applicationContext = null) {
    if (lifecycle.ownerApplyChatEvent) return;
  ...
  function applyEventToLifecycle(lifecycle, event, ws, applicationContext = null) {
    if (lifecycle.applicationExpired || !lifecycle.ingressOpen) {
      onDiagnostic('agent_turn_late_event');
      return lifecycle.finalization;
    }
    claimOwner(lifecycle, ws, applicationContext);
  ...
  function beginTurn(event, ws) {
    ...
    const start = () => {
      ...
      return applyEventToLifecycle(lifecycle, event, ws);
    };
  ...
  function applyHarnessEvent(event, ws) {
    ...
    if (event.type === 'turn_begin') return beginTurn(event, ws);
    if (event.type === 'turn_end') return finalizeTurn(event, ws);
    ...
    if (!lifecycle.queuePending) {
      ...
      lifecycle.finalization = Promise.resolve().then(
        () => ownerApplyChatEvent(
          ...
          ownerWs,
          lifecycle.ownerApplicationContext,
        ),
      );
  ...
  function finalizeTurn(event, ws, identityOverride = null) {
  ...
  async function drainHarnessEvents(events, ws, options = {}) {
    ...
    let lifecycle = capturedKey
      ...
      : null;
    if (lifecycle) claimOwner(lifecycle, ws, options.turnApplicationContext || null);
    ...
    function applyBoundEvent(event) {
      ...
      return applyEventToLifecycle(bound, event, ws, options.turnApplicationContext || null);
    }
