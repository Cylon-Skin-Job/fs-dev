...
function validRequestId(value) { return scalarStringWithin(value, 128); }

function validEnvelope(message) {
  return message?.type === 'agent:activity:query'
    && message.version === 1
...
function error(code, extra = {}) {
  return Object.freeze({ type: 'agent:activity:error', version: 1, code, ...extra });
}
...
function createAgentActivityRoute({
  registryAccess,
  repository,
  ...
  if (typeof registryAccess?.validatePayload !== 'function') throw new TypeError('registry validation access is required');
  if (typeof repository?.query !== 'function') throw new TypeError('agent activity repository is required');
  ...
  function closeInvalidAuthority(ws) {
    diagnose('agent_activity_schema_unavailable');
    try { ws.close(1011, 'agent activity authority unavailable'); } catch (_error) {}
  }

  async function sendFixedQueryFailure(ws, session, requestId, pair) {
    diagnose('agent_activity_query_failed');
    try {
      return await sendReply(ws, session, error('query_failed', { requestId, ...pair }), pair);
    } catch (_error) {
      try { ws.close(1011, 'agent activity query failed'); } catch (_closeError) {}
      return false;
    }
  }

  async function sendValidated(ws, session, value, pair = null) {
    let validation;
    try {
    ...
    return sendReply(ws, session, value, pair);
  }
  ...
  async function handleValidatedQuery({ ws, session, message }) {
    ...
    if (message.workspaceId !== pair.workspaceId || message.workspaceEpoch !== pair.workspaceEpoch) {
      return sendValidated(ws, session, error('stale_workspace', { requestId: message.requestId, ...pair }), pair);
    }
    ...
    try {
      result = await repository.query({
        ...
        ...(normalizedPaths.canonicalPath == null ? {} : { path: normalizedPaths.canonicalPath }),
        ...(normalizedPaths.folderPrefix == null ? {} : { folderPrefix: normalizedPaths.folderPrefix }),
        ...Object.fromEntries([
          'fileName', 'activityId', 'threadId', 'turnId', 'exchangeId', 'toolCallId', 'harnessId',
          'toolNames', 'statuses', 'accessFamilies', 'accessKinds', 'changedOnly', 'since', 'until',
          'cursor', 'limit',
  ...
  async function handleQuery(input) {
    try {
      ...
      if (requestId && pair) return sendFixedQueryFailure(input.ws, input.session, requestId, pair);
      diagnose('agent_activity_query_failed');
      try { input?.ws?.close(1011, 'agent activity query failed'); } catch (_closeError) {}
      return false;
    }
  }
...
module.exports = { SCHEMA_REFERENCE, createAgentActivityRoute, validRequestId };
