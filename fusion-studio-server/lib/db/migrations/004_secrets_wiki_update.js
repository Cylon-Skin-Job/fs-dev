 ...
 *
 * All six pages: slim user-facing content (the user has the assistant to help),
 * dense structured context for assistant retrieval.
 */
...
A CLI is an AI assistant that runs on your machine. It's the engine behind your conversations and agent tasks. Fusion Studio connects to it and displays the results.
...
Connectors give your AI access to apps on your Mac — Mail, Calendar, Reminders, Notes, and more. They're built into Fusion Studio and ready to use once you turn them on.
...
Safety rules control what your AI can and can't do. They're built into Fusion Studio and can only be changed by you from this panel. Your AI cannot modify, disable, or work around them.

## Can I change these rules?

Some rules have a toggle. The defaults are designed to prevent common mistakes — like an AI overwriting its own configuration. You can relax a rule if you want, but Fusion Studio will make sure you understand what you're changing first.

## What gets logged?

Fusion Studio can record what happens in the system — file changes, agent actions, trigger fires. You can turn logging and notifications on or off here.`,
...
exports.down = async function (knex) {
  ...
  await knex('system_wiki')
    .where('slug', 'clis')
    .update({
      content: '## What is a CLI?\n\nA CLI (command-line interface) is an AI assistant that runs on your machine. It\'s the engine that powers your conversations and agent tasks. Fusion Studio doesn\'t process AI requests itself — it connects to a CLI and displays the results.\n\nThink of Fusion Studio as the dashboard and the CLI as the engine under the hood. You can swap engines anytime.\n\n## Why does Fusion Studio use a CLI?\n\nFusion Studio is a harness, not an AI. It reads what your CLI sends back and displays it in a visual interface you can manage. This means you\'re never locked into one provider — you can switch CLIs, use different ones for different tasks, or try a new one without changing anything about your project.\n\nYour CLI handles the AI work. Fusion Studio handles everything else: organizing conversations, managing files, running triggers, and keeping your project in order.\n\n## Can I switch between CLIs?\n\nYes. You can have multiple CLIs installed and switch between them. Each CLI has its own configuration, but your project, conversations, and settings stay the same regardless of which CLI is active.\n\nSome people use one CLI for everyday coding and another for research or analysis. Fusion Studio makes it easy to swap — just change the active CLI in this panel.\n\n## Will using a CLI cost me money?\n\nFusion Studio is completely free. It will never charge you anything.\n\nThe AI work itself — the conversations, the code generation, the analysis — that\'s handled by whichever CLI you choose. How you pay for that is entirely up to you. Most CLI providers offer metered plans where you sign in and get a monthly token allocation, often at rates significantly lower than raw API pricing. Some, like Qwen and Gemini, even have generous free tiers.\n\nYou can also bring your own API keys from any provider and configure your CLI to use them directly. Either way, you\'re buying tokens from the AI provider of your choice — not from us. Fusion Studio simply sits in the middle and tells you what\'s happening.',
      context: 'CLI = command-line AI assistant binary installed locally. Fusion Studio reads the wire protocol (RPC) and renders output. The CLI handles all AI inference. Examples: kimi, claude, qwen, codex, gemini, opencode. User must have at least one installed. Fusion Studio is a harness/display layer — no lock-in, any CLI, any API key, any provider. Multiple CLIs can be installed simultaneously. Active CLI set per-system in fusion.db. Switching CLIs does not affect project state, chat history, triggers, or settings. Fusion Studio is free, charges nothing. Token costs go to the CLI provider. Metered CLI plans are often cheaper than raw API pricing. Free tiers: Qwen ~2000 req/day, Gemini ~1000 req/day. BYO API keys also supported.',
      description: 'What CLIs are, why Fusion Studio uses them, switching, and cost',
  ...
  await knex('system_wiki')
    .where('slug', 'secrets')
    .update({
      content: '## What is the secrets manager?\n\nThe secrets manager stores sensitive information like API keys, access tokens, and passwords. Everything is encrypted and stored locally on your machine in Fusion Studio\'s database. Secrets are used by connectors and CLI configurations behind the scenes.\n\nYou can add, update, or remove secrets anytime from this panel.\n\n## Can AI agents see my secrets?\n\nNo. This is a hard rule. AI agents never have access to your secrets, API keys, or tokens. The enforcement system prevents it. When a connector needs a token to sync with GitLab, Fusion Studio\'s server process uses it directly — the AI never sees it.\n\nThis is by design, not configuration. You can\'t accidentally expose secrets to an agent.',
      context: 'Secrets = encrypted key-value store in fusion.db. Holds: API keys, access tokens, passwords. Used by connectors and CLI configs. Never exposed to AI agents — enforcement rule. Only Fusion Studio server process reads them for connector/CLI auth. Stored locally, never synced. Hard enforcement: AI agents NEVER get secret access. This is a security boundary, not a preference.',
  ...
  await knex('system_wiki')
    .where('slug', 'connectors')
    .update({
      content: '## What are connectors?\n\nConnectors let Fusion Studio talk to external services you already use — like GitLab, GitHub, or Jira. When a connector is active, Fusion Studio can pull in issues, sync ticket status, and keep your external tools up to date with what\'s happening in your project.\n\nConnectors run on your machine and use your credentials. Nothing is sent to Fusion Studio\'s servers because there aren\'t any.\n\n## Is my data shared externally?\n\nNo. Connectors run entirely on your machine. When Fusion Studio syncs with GitLab, for example, it\'s your machine talking directly to GitLab using your credentials. Fusion Studio has no servers, no cloud, no telemetry. Your data stays between your machine and the services you choose to connect.',
      context: 'Connectors = integrations with external services (GitLab, GitHub, Jira, etc.). Run locally, use user credentials stored in secrets manager. Sync is bidirectional where supported. No Fusion Studio servers involved — everything local. Zero data sharing. No telemetry, no cloud sync, no analytics. Fully local architecture.',
